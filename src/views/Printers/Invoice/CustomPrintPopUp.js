...
const CustomPrintPopUp = (props) => {
  const classes = useStyles();
  const { fullWidth, maxWidth } = props;

  const store = useStore();
  const { openCustomPrintPopUp } = toJS(store.PrinterSettingsStore);
  const { handleCloseCustomPrintPopUp } = store.PrinterSettingsStore;
  ...
  const saveDataOnClick = () => {
    ...
    window.localStorage.setItem('onCustomPrintPopupData', JSON.stringify(invoiceRegular));
    const myEvent = new CustomEvent('onCustomPrintPopupEvent', {
      data: invoiceRegular
    });
    // dispatch the event
  ...
  return (
    <div>
      <Dialog
        open={openCustomPrintPopUp}
        onClose={handleCloseCustomPrintPopUp}
        fullWidth={fullWidth}
        maxWidth={maxWidth}
      >
...
export default InjectObserver(CustomPrintPopUp);