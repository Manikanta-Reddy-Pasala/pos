...
const SalesReturnByCustomer = (props) => {
  ...
  const getDataFromSalesReturnByCustomer = async () => {
    const wb = new Workbook();

    let xlsxReturnDetailsList = [];

    if (customerId) {
      xlsxReturnDetailsList = await getAllSaleReturnDataByDateAndCustomerXlsx(
        fromDate,
        toDate,
        customerId
      );
    ...
    wb.SheetNames.push('Sales Return By Customer Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Sales Return By Customer Sheet'] = ws;
    ...
    const fileName = 'Sales_Return_By_Customer_Report';
  ...
  const handleSearch = async (e) => {
    if (e) {
      ...
      if (target) {
        if (customerId) {
          getSalesReturnSearchWithCustomerAndDate(
            target,
            fromDate,
            toDate,
            customerId
          ...
          getSalesReturnSearchWithDate(target, fromDate, toDate);
        }
      } else {
        if (customerId) {
          getSaleReturnDataByDateAndCustomer(fromDate, toDate, customerId);
        } else {
  ...
  const handleCustomerClick = (customer) => {
    ...
    getSaleReturnDataByDateAndCustomer(fromDate, toDate, customer.id);
    setcustomerList([]);
  };
  ...
  const getSalesReturnSearchWithDate = async (value, fromDate, toDate) => {
    const db = await Db.get();
    ...
    if (currentPage && currentPage > 1) {
      ...
      getAllSalesReturnSearchWithDate(value, fromDate, toDate);
    }
    const businessData = await Bd.getBusinessData();
  ...
  const getAllSalesReturnSearchWithDate = async (value, fromDate, toDate) => {
    const db = await Db.get();
  ...
  const getSalesReturnSearchWithCustomerAndDate = async (
    value,
    fromDate,
    toDate,
    customerId
    ...
    if (currentPage && currentPage > 1) {
      ...
      getAllSalesReturnSearchWithCustomerAndDate(value, fromDate, toDate);
    }
    const businessData = await Bd.getBusinessData();
  ...
  const getAllSalesReturnSearchWithCustomerAndDate = async (
    value,
    fromDate,
    toDate,
    customerId
  ...
  const getSaleReturnDataByDateAndCustomer = async (
    fromDate,
    toDate,
    customerId
  ) => {
    ...
    if (currentPage && currentPage > 1) {
      ...
      getAllSaleReturnDataByDateAndCustomer(fromDate, toDate, customerId);
    }
    const businessData = await Bd.getBusinessData();
  ...
  const getAllSaleReturnDataByDateAndCustomer = async (
    fromDate,
    toDate,
    customerId
  ) => {
  ...
  const getAllSaleReturnDataByDateAndCustomerXlsx = async (
    fromDate,
    toDate,
    customerId
  ) => {
                ...
                >
                  <Grid container direction="row" alignItems="center">
                    ...
                    <Grid xs={2} className="category-actions-right">
                      <Avatar>
                        <IconButton
                          onClick={() => getDataFromSalesReturnByCustomer()}
                        >
...
export default InjectObserver(SalesReturnByCustomer);