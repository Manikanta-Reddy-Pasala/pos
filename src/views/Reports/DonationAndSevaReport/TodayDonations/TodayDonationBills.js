...
const TodayDonationBills = (props) => {
  ...
  const getDataFromSales = async () => {
    ...
    if (allSalesData && allSalesData.length > 0) {
      for (var i = 0; i < allSalesData.length; i++) {
        const record = {
          ...
          'INVOICE NUMBER': allSalesData[i].invoice_number,
    ...
    wb.SheetNames.push('Today Donation Bills Sheet');

    // console.log('test:: ws::', ws);
    wb.Sheets['Today Donation Bills Sheet'] = ws;
    ...
    const fileName = 'Today_Donation_Bills_Report';
...
export default InjectObserver(TodayDonationBills);
