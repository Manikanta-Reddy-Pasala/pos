...
const TotalApprovalBills = (props) => {
  ...
  const getDataFromApprovals = async () => {
    ...
    wb.SheetNames.push('All Approval Bills Sheet');

    wb.Sheets['All Approval Bills Sheet'] = ws;
    ...
    const fileName = 'All_Approval_Bills_Report';
  ...
  const getApprovalDataByDate = async (fromDate, toDate) => {
    const db = await Db.get();
    var Query;
...
export default InjectObserver(TotalApprovalBills);
