...
import {
  getSalesTxnByModelNo,
  getSaleTxnByCategoryAndModelNo,
  getSalesTxnByAllModelNo
...
const SalesByModelNo = () => {
  ...
  const getDataFromSalesByItemReport = () => {
    ...
    wb.SheetNames.push('Sales By Model Report Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Sales By Model Report Sheet'] = ws;
    ...
    const fileName = 'Purchases_By_Model_Report';
  ...
  const handleChangeSubCategory = async (event) => {
    ...
    const db = await Db.get();
    let txnData = await getSaleTxnByCategoryAndModelNo(
      db,
      level2Category.name,
      event.name,
      fromDate,
  ...
  const resetCategory = async (event) => {
    ...
    let txnData;
    if (modelNo !== '') {
      txnData = await getSalesTxnByModelNo(
        db,
        fromDate,
        toDate,
        getFilterArray(),
      ...
      txnData = await getSalesTxnByAllModelNo(
        db,
        fromDate,
        toDate,
        getFilterArray()
  ...
  async function fetchData() {
    ...
    if (level2Category.name && level3Category.name) {
      ...
      let txnData = await getSalesTxnByModelNo(
        db,
        fromDate,
        toDate,
        getFilterArray(),
      ...
      let txnData = await getSalesTxnByAllModelNo(
        db,
        fromDate,
        toDate,
        getFilterArray()
...
export default InjectObserver(SalesByModelNo);