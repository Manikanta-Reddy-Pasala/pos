...
import {
  getSalesTxnByFreeProduct,
  getFreeSaleTxnByCategory,
  getFreeSalesTxn
...
const SaleByFreeItem = () => {
  ...
  const getDataFromSalesByItemReport = () => {
    ...
    wb.SheetNames.push('Sales By Free Product Report Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Sales By Free Product Report Sheet'] = ws;
    ...
    const fileName = 'Sales_By_Free_Product_Report';
  ...
  const handleChangeSubCategory = async (event) => {
    ...
    const db = await Db.get();
    setRowData(
      await getFreeSaleTxnByCategory(
        db,
        level2Category.name,
        event.name,
        fromDate,
  ...
  const resetCategory = async (event) => {
    ...
    setRowData(await getFreeSalesTxn(db, fromDate, toDate));
  };
  ...
  async function fetchData() {
    ...
    if (level2Category.name && level3Category.name) {
      ...
      if (selectedProduct && selectedProduct.productId) {
        setRowData(
          await getSalesTxnByFreeProduct(
            db,
  ...
  const handleProductClick = async (option) => {
    ...
    setRowData(
      await getSalesTxnByFreeProduct(db, option.productId, fromDate, toDate)
    );
    setproductlist([]);
  };
...
export default InjectObserver(SaleByFreeItem);
