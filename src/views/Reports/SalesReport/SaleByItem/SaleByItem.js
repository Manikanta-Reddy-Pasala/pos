...
import { InvoiceThermalReportPrintContent } from 'src/views/Printers/ComponentsToPrint/invoiceThermalReportPrintContent';
import { Print } from '@material-ui/icons';
import { printThermal } from 'src/views/Printers/ComponentsToPrint/printThermalContent';
import PDFIcon from '@material-ui/icons/PictureAsPdf';
import { pdf } from '@react-pdf/renderer';
import SaleProductPDF from 'src/views/PDF/SaleProduct/SaleProductPDF';
...
const SaleByItem = () => {
  ...
  const resetCategory = async (event) => {
    ...
    setRowData(await getSalesTxn(db, fromDate, toDate, getFilterArray()));
  };
  ...
  const onReportPrintClicked = async () => {
    if (invoiceThermal.boolDefault) {
      let data = rowData;
      let total = 0;
      let totalQty = 0;
      for (let item of rowData) {
        total += parseFloat(item.saleAmount);
        totalQty += parseFloat(item.saleQty);
      }
      const printContent = InvoiceThermalReportPrintContent(
        invoiceThermal,
        data,
        fromDate,
        toDate,
            ...
            <Paper className={classes.root} style={{ minHeight: height - 83 }}>
              ...
              <div>
                ...
                >
                  ...
                  {localStorage.getItem('isHotelOrRestaurant') === 'true' && (
                    <Grid item xs={1} style={{ marginTop: 'auto'}}>
                      <Grid
                        container
                        direction="row"
                        alignItems="center"
                      ...
                      >
                        <Avatar>
                          <IconButton onClick={() => onReportPrintClicked()}>
...
export default InjectObserver(SaleByItem);