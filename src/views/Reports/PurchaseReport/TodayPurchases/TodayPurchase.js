...
const TodayPurchase = (props) => {
  ...
  const getDataFromPurchases = async () => {
    ...
    if (xlsxPurchaseData && xlsxPurchaseData.length > 0) {
      for (var i = 0; i < xlsxPurchaseData.length; i++) {
        const record = {
          ...
          'BILL VALUE': xlsxPurchaseData[i].total_amount,
          'TOTAL TAX %': getTotalTaxPercent(xlsxPurchaseData[i]),
          'TAXABLE VALUE': getTaxableValue(xlsxPurchaseData[i]),
          SGST: getTotalSgst(xlsxPurchaseData[i]),
          CGST: getTotalCgst(xlsxPurchaseData[i]),
          IGST: getTotalIgst(xlsxPurchaseData[i]),
          CESS: getTotalCess(xlsxPurchaseData[i]),
          'TOTAL TAX': getTotalTax(xlsxPurchaseData[i]),
          'TOTAL DISCOUNT': getTotalDiscount(xlsxPurchaseData[i]),
    ...
    wb.SheetNames.push('Today Purchases Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Today Purchases Sheet'] = ws;
    ...
    const fileName = 'Today_Purchases_Report';
  ...
  const { handlePurchasesSearchWithDate } = stores.PurchasesAddStore;
...
export default InjectObserver(TodayPurchase);