...
const PurchaseByVendor = (props) => {
  ...
  const getDataFromPurchasesByVendor = async () => {
    ...
    wb.SheetNames.push('Purchases By Vendor Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Purchases By Vendor Sheet'] = ws;
    ...
    const fileName = 'Purchases_By_Vendor_Report';
  ...
  const getAllPurchaseDataByVendor = async (fromDate, toDate, vendorId) => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
  ...
  const getAllPurchaseDataByVendorXlsx = async (fromDate, toDate, vendorId) => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
  ...
  return (
    <div>
      ...
      {!isLoading && (
        <div className={classes.root} style={{ minHeight: height - 90 }}>
          {isFeatureAvailable ? (
            <Paper className={classes.root} style={{ minHeight: height - 90 }}>
...
export default InjectObserver(PurchaseByVendor);