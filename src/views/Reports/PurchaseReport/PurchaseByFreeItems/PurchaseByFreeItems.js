...
import {
  getPurchasesTxnByFreeProduct,
  getPurchasesTxnByFreeCategory,
  getFreePurchasesTxn
...
const PurchaseByFreeItems = () => {
  ...
  const getDataFromPurchasesByItemReport = () => {
    ...
    wb.SheetNames.push('Purchases By Free Product Sheet');

    // console.log('test:: ws::', ws);
    wb.Sheets['Purchases By Free Product Sheet'] = ws;
  ...
  const handleChangeSubCategory = async (event) => {
    ...
    const db = await Db.get();
    setRowData(
      await getPurchasesTxnByFreeCategory(
        db,
        level2Category.name,
        event.name,
        fromDate,
  ...
  const handleProductClick = async (option) => {
    ...
    setRowData(
      await getPurchasesTxnByFreeProduct(db, option.productId, fromDate, toDate)
    );
    setproductlist([]);
  };
  ...
  async function fetchData() {
    ...
    if (level2Category.name && level3Category.name) {
      ...
      if (selectedProduct && selectedProduct.productId) {
        setRowData(
          await getPurchasesTxnByFreeProduct(
            db,
        ...
        setRowData(await getFreePurchasesTxn(db, fromDate, toDate));
      }
    }
  }
...
export default InjectObserver(PurchaseByFreeItems);