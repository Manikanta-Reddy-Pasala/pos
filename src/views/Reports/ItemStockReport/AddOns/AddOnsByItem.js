...
import { getAddOnProductTxnListByDateAndProduct } from 'src/components/Helpers/ProductTxnQueryHelper';
...
const AddOnsByItem = () => {
  ...
  const [columnDefs] = useState([
    ...
    },
    {
      ...
      valueFormatter: (params) => {
        ...
        let result = parseFloat(data['txnQty'] || 0).toFixed(2);
        return result;
      },
  ...
  const getDataFromSalesByItemReport = () => {
    ...
    wb.SheetNames.push('Add Ons Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Add Ons Sheet'] = ws;
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setRowData(
        await getAddOnProductTxnListByDateAndProduct(db, fromDate, toDate)
      );
    }
  ...
  async function fetchData() {
    ...
    if (selectedProduct && selectedProduct.productId) {
      if (selectedProduct && selectedProduct.productId) {
        setRowData(
          await getAddOnProductTxnListByDateAndProduct(
            db,
            fromDate,
            toDate,
...
export default InjectObserver(AddOnsByItem);