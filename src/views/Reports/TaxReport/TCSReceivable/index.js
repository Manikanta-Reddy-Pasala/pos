...
const TCSReceivable = (props) => {
  ...
  const getDataFromPurchases = async () => {
    ...
    if (xlsxPurchaseData && xlsxPurchaseData.length > 0) {
      for (var i = 0; i < xlsxPurchaseData.length; i++) {
        const record = {
          ...
          'TCS PAID': parseFloat(xlsxPurchaseData[i].tcsAmount).toFixed(2),
          'TCS CODE': xlsxPurchaseData[i].tcsCode,
          'TCS NAME': xlsxPurchaseData[i].tcsName,
          'TCS RATE(%)': xlsxPurchaseData[i].tcsRate
        };
        data.push(record);
      }
    } else {
    ...
    wb.SheetNames.push('TCS Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['TCS Sheet'] = ws;
    ...
    const fileName = 'TCS_Receivable_Report';
  ...
  const { handlePurchasesSearchWithDateAndTCS } = stores.PurchasesAddStore;
...
export default InjectObserver(TCSReceivable);