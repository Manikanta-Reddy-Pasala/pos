...
import TDSReceivablePDF from 'src/views/PDF/TDSReceivable/TDSReceivablePDF';
...
const TDSReceivable = (props) => {
  ...
  function dateFormatterPDF(date) {
  ...
  const getDataFromPurchases = async () => {
    ...
    const fileName = 'TDS_Receivable_Report';
  ...
  const generatePDFDocument = async () => {
    ...
    const blob = await pdf(
      <TDSReceivablePDF
        data={xlsxPurchaseData}
        settings={invoiceRegular}
        date={dateFormatterPDF(fromDate) + ' to ' + dateFormatterPDF(toDate)}
      />
    ).toBlob();

    console.log(blob);

    saveAs(blob, 'TDS_Receivable');
  };
...
export default InjectObserver(TDSReceivable);