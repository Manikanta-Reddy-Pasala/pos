...
const PurchasewithHSNGstReport = () => {
  ...
  const { getPurchaseRowData, getPurchaseData } = store.GSTHsnStore;
  ...
  useEffect(() => {
    async function fetchData() {
      await getPurchaseData(fromDate, toDate);
      setRowData(await getPurchaseRowData());
  ...
  }, [fromDate, getPurchaseData, getPurchaseRowData, toDate]);
  ...
  const getDataFromTax = () => {
    ...
    if (rowData.length > 0) {
      for (var i = 0; i < rowData.length; i++) {
        ...
        const record = {
          ...
          'Party GSTIN': rowData[i].vendorGSTNo,
          'Party Name': rowData[i].vendorName,
          'Product Name': rowData[i].productName,
          'HSN Code': rowData[i].hsn,
          Qty: rowData[i].txnQty,
          'Price/Unit': rowData[i].purchasedPrice,
          'Tax Rate': rowData[i].total_tax_rate,
          'Taxable Value': parseFloat(result).toFixed(2),
          SGST: rowData[i].sgst_amount,
          CGST: rowData[i].cgst_amount,
          IGST: rowData[i].igst_amount,
    ...
    wb.SheetNames.push('Purchase(with HSN) Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Purchase(with HSN) Sheet'] = ws;
    ...
    const fileName = 'Purchase_With_HSN_Report';
...
export default InjectObserver(PurchasewithHSNGstReport);