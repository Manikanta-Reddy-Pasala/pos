...
const SalewithHSNGstReport = () => {
  ...
  const { getSalesData } = store.GSTHsnStore;
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setRowData(await getSalesData(fromDate, toDate));
    }

    fetchData();
    setTimeout(() => setLoadingShown(false), 200);
  }, [fromDate, getSalesData, toDate]);
  ...
  const getDataFromTax = () => {
    ...
    if (rowData && rowData.length > 0) {
      for (var i = 0; i < rowData.length; i++) {
        ...
        const record = {
          ...
          'Party GSTIN': rowData[i].customerGSTNo,
          'Party Name': rowData[i].customerName,
          'Product Name': rowData[i].productName,
          'HSN Code': rowData[i].hsn,
          Qty: rowData[i].txnQty,
          'Price/Unit': rowData[i].salePrice,
    ...
    wb.SheetNames.push('GST Sales(HSN) Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['GST Sales(HSN) Sheet'] = ws;
    ...
    const fileName = 'GST_Sales_with_Sales_Report';
...
export default InjectObserver(SalewithHSNGstReport);