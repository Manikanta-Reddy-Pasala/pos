...
const BillWiseDiscountPurchases = (props) => {
  ...
  const getDataFromPurchases = async () => {
    ...
    wb.SheetNames.push('Bill Wise Disc Purchase Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Bill Wise Disc Purchase Sheet'] = ws;
    ...
    const fileName = 'Bill_Wise_Disc_Purchase_Report';
  ...
  const handleSearch = async (e) => {
    if (e) {
      ...
      if (target) {
        if (fromDate && toDate) {
          getPurchaseDataBySearch(target, fromDate, toDate);
        }
      } else {
        if (fromDate && toDate) {
  ...
  const getPurchaseDataBySearch = async (value, fromDate, toDate) => {
    const db = await Db.get();
    var Query;
    ...
    if (currentPage && currentPage > 1) {
      ...
      getAllPurchaseDataBySearch(value, fromDate, toDate);
    }
    const businessData = await Bd.getBusinessData();
  ...
  const getAllPurchaseDataBySearch = async (value, fromDate, toDate) => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
...
export default InjectObserver(BillWiseDiscountPurchases);