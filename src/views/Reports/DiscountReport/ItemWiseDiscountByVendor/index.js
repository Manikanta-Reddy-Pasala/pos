...
import { getDiscountTxnForPurchases } from 'src/components/Helpers/ProductTxnQueryHelper';
...
const ItemWiseDiscountVendor = () => {
  ...
  const [columnDefs] = useState([
    ...
    },
    {
      ...
      valueFormatter: (params) => {
        let data = params['data'];
        let result = 0;
        if (data['purchaseQty'] && data['purchaseQty'] > 0) {
          result =
            parseFloat(data['discountAmount'] || 0) / data['purchaseQty'];
        } else {
  ...
  function getAvgDiscountData(data) {
    let result = parseFloat(data['discountAmount'] || 0) / data['purchaseQty'];
    return parseFloat(result).toFixed(2);
  }
  ...
  const getDataFromSalesByItemReport = () => {
    ...
    if (rowData && rowData.length > 0) {
      for (var i = 0; i < rowData.length; i++) {
        const record = {
          ...
          'PURCHASE AMOUNT': parseFloat(rowData[i].purchaseAmount || 0).toFixed(
            2
          ),
    ...
    wb.SheetNames.push('Item Wise Disc Vendor Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Item Wise Disc Vendor Sheet'] = ws;
    ...
    const fileName = 'Item_Wise_Disc_Vendor_Report';
  ...
  const handleChangeSubCategory = async (event) => {
    ...
    const db = await Db.get();
    let txnData = await getDiscountTxnForPurchases(
      db,
      fromDate,
      toDate,
              ...
              <div className={classes.content}>
                <div className={classes.contentLeft}>
                  <Typography gutterBottom variant="h4" component="h4">
                    PRODUCT WISE DISCOUNT BY VENDOR
                  </Typography>
                </div>
              </div>
...
export default InjectObserver(ItemWiseDiscountVendor);