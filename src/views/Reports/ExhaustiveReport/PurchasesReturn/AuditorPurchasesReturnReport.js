...
const AuditorPurchasesReport = (props) => {
  ...
  function getColumnDefs() {
    ...
    columnDefs.push(purchaseInvoiceNo);
    columnDefs.push(purchaseInvoiceDate);
    columnDefs.push(purchaseInvoiceValue);
  ...
  const getDataFromPurchases = async () => {
    ...
    if (xlsxData && xlsxData.length > 0) {
      for (let i = 0; i < xlsxData.length; i++) {
        ...
        record['DN NO'] = xlsxData[i].purchaseReturnBillNumber;
        ...
        record['PURCHASE INV NUMBER'] = xlsxData[i].vendor_bill_number;
        record['PURCHASE INV DATE'] = formatDownloadExcelDate(
          xlsxData[i].purchaseDate
        );
        record['PURCHASE INV VALUE'] = xlsxData[i].purchaseTotalAmount;
      ...
      record['GSTIN/UIN'] = '';
      record['TOTAL VALUE'] = '';
      record['PURCHASE INV NUMBER'] = '';
      record['PURCHASE INV DATE'] = '';
      record['PURCHASE INV VALUE'] = '';
    ...
    wb.SheetNames.push('All Purchase Returns Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['All Purchase Returns Sheet'] = ws;
    ...
    const fileName = 'All_Purchase_Returns_Report';
  ...
  const getPurchasesSearchWithDate = async (value, fromDate, toDate) => {
    ...
    let query = await db.purchasesreturn.find({
      selector: {
        $or: [
          {
            $and: [
