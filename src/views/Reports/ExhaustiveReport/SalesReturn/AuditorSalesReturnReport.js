...
const AuditorSalesReturnReport = (props) => {
  ...
  const { openAddSalesReturn, salesReportFilters } = toJS(
  ...
  const handleCellClicked = (event) => {
    ...
    } else if ('saleSequenceNumber' === colId) {
      viewOrEditSaleTxnItem(event.data);
    }
  };
  ...
  function getColumnDefs() {
    ...
    const saleInvoiceValue = {
      field: 'saleTotalAmount',
      filterParams: {
        buttons: ['reset', 'apply'],
        closeOnApply: true
      },
    ...
    columnDefs.push(saleInvoiceNo);
    columnDefs.push(saleInvoiceDate);
    columnDefs.push(saleInvoiceValue);
  ...
  const getDataFromSales = async () => {
    ...
    if (xlsxData && xlsxData.length > 0) {
      for (let i = 0; i < xlsxData.length; i++) {
        ...
        record['CRN NO'] = xlsxData[i].sequenceNumber;
        record['DATE'] = formatDownloadExcelDate(xlsxData[i].date);
        record['CUSTOMER NAME'] = xlsxData[i].customer_name;
        record['GSTIN/UIN'] = xlsxData[i].customerGSTNo;
        record['PLACE OF SUPPLY'] = xlsxData[i].customerState;
        record['CREDIT NOTE VALUE'] = xlsxData[i].total_amount;
        ...
        record['SALE INV NUMBER'] = xlsxData[i].saleSequenceNumber;
        record['SALE INV DATE'] = formatDownloadExcelDate(
          xlsxData[i].invoice_date
        );
        record['SALE INV VALUE'] = xlsxData[i].saleTotalAmount;
        record['ROUND OFF'] = xlsxData[i].round_amount;

        record['CASH'] = xlsxData[i].cash;
        record['SALE ADJUST'] = xlsxData[i].creditNote;
        ...
        record['GIFT CARD'] = xlsxData[i].giftCard;
        record['CUSTOM FINANCE'] = xlsxData[i].customFinance;
      ...
      record['SALE INV NUMBER'] = '';
      record['SALE INV DATE'] = '';
      record['SALE INV VALUE'] = '';
    ...
    wb.SheetNames.push('All Sales Return Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['All Sales Return Sheet'] = ws;
    ...
    const fileName = 'All_Sales_Return_Report';
...
export default InjectObserver(AuditorSalesReturnReport);