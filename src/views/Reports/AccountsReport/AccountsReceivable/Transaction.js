...
const TransactionTable = () => {
  ...
  const getDataForExcel = () => {
    ...
    let data = [];
    if (customerTransactionList && customerTransactionList.length > 0) {
      for (var i = 0; i < customerTransactionList.length; i++) {
        const record = {
          TYPE: customerTransactionList[i].txnType,
          DATE: getDateFormat(customerTransactionList[i]),
          'REF NO.': customerTransactionList[i].sequenceNumber,
          TOTAL: customerTransactionList[i].amount,
          BALANCE: customerTransactionList[i].balance,
          STATUS: getStatus(customerTransactionList[i])
        };
        data.push(record);
      }
    } else {
                    ...
                    <Controls.Input
                      ...
                      onChange={async (event) => {
                        if (event.target.value.toString().toLowerCase()) {
                          setRowData(
                            allTxn.searchTransactionListByPartyIdAndValue(
                              customer.id,
