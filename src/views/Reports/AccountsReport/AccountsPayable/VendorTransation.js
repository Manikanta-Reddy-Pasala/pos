...
const VendorTransactionTable = () => {
  ...
  const [fromDate, setFromDate] = React.useState(
    dateHelper.formatDateToYYYYMMDD(dateHelper.getFinancialYearStartDate())
  );
  const [toDate, setToDate] = React.useState(
    dateHelper.formatDateToYYYYMMDD(dateHelper.getFinancialYearEndDate())
  );
  ...
  const { vendor, changeInVendor, vendorTransactionList, selectedParty } = toJS(
    store.VendorStore
  );
  ...
  const getDataForExcel = () => {
    ...
    let data = [];
    if (vendorTransactionList && vendorTransactionList.length > 0) {
      for (var i = 0; i < vendorTransactionList.length; i++) {
        const record = {
          TYPE: vendorTransactionList[i].txnType,
          DATE: getDateFormat(vendorTransactionList[i]),
          'REF NO.': vendorTransactionList[i].sequenceNumber,
          TOTAL: vendorTransactionList[i].amount,
          BALANCE: vendorTransactionList[i].balance,
          STATUS: getStatus(vendorTransactionList[i])
        };
        data.push(record);
      }
    } else {
    ...
    wb.SheetNames.push('Vendor Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Vendor Sheet'] = ws;
    ...
    const fileName = vendor.name + '_Transactions';
  ...
  const generatePDFDocument = async () => {
    ...
    saveAs(blob, vendor.name + '_Transactions');
  };
                    ...
                    <Controls.Input
                      ...
                      onChange={async (event) => {
                        if (event.target.value.toString().toLowerCase()) {
                          ...
                          allTxn.getAllTransactionListByPartyId(vendor.id);
                        }
                      }}
                    />
                  </Grid>
        ...
        <Grid item xs={12}>
          <Box mt={4} style={{ marginTop: '0px' }}>
            <div
              ...
              className="red-theme "
              style={{ width: '100%', height: height - 273 + 'px' }}
            >
              <div
                id="product-list-grid"
