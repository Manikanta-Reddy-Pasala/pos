...
const VendorLedger = () => {
  ...
  const getDataForExcel = () => {
    ...
    if (vendorLedgerTransactionList && vendorLedgerTransactionList.length > 0) {
      for (var i = 0; i < vendorLedgerTransactionList.length; i++) {
        const record = {
          DATE: getDateFormat(vendorLedgerTransactionList[i]),
          'REF NO.': vendorLedgerTransactionList[i].sequenceNumber,
          TYPE: vendorLedgerTransactionList[i].txnType,
          DEBIT: vendorLedgerTransactionList[i].debit,
          CREDIT: vendorLedgerTransactionList[i].credit,
          BALANCE: vendorLedgerTransactionList[i].balance
        };
        data.push(record);
      }
    } else {
  ...
  const updateEnableBalance = (isVisible) => {
    ...
    const updatedColumnDefs = columnDefs.map((columnDef) => {
      if (columnDef.field === 'totalBalance') {
