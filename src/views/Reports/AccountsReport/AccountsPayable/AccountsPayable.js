...
import VendorLedger from 'src/views/Vendors/VendorLedger';
import VendorTransaction from 'src/views/Vendors/VendorTransation';
...
const AccountPayableReport = () => {
  ...
  const localenablePayableBalance = localStorage.getItem(
    'enablePayableBalance'
  );
  ...
  const { setVendorFromDate, setVendorToDate,setSelectedVendor } = store.VendorStore;

  useEffect(() => {
    getInvoiceSettings(localStorage.getItem('businessId'));

    if (localenablePayableBalance === 'true') {
  ...
  const getDataFromCashFlow = () => {
    ...
    wb.SheetNames.push('Accounts Payable Sheet');

    wb.Sheets['Accounts Payable Sheet'] = ws;
    ...
    const fileName = 'Accounts_Payable_Sheet';
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let result = await getAllLedgerTransactionListDateRangeAndHavingBalance(
        ...
        toDate,
        null, // search value
        'payable' //ledger type payable or receivable
      );
  ...
  const generatePDFDocument = async () => {
    ...
    const blob = await pdf(
      <AccountsReceivableSundryPDF
        ...
        toDate={toDate}
        isPayable={false}
        title={'Accounts Payable/Sundry Creditors'}
        type={'Payable'}
  ...
  const updateEnableBalance = (isVisible) => {
    ...
    setGridKey((prevKey) => prevKey + 1);
    localStorage.setItem('enablePayableBalance', isVisible);
  };
                ...
                {displayMode === 'all' ? (
                  ...
                  <div className={classes.itemTable}>
                    ...
                    <TabPanel value={Tabvalue} index={0}>
                      <VendorTransaction fromDate={fromDate} toDate={toDate} data="payable" />
                    </TabPanel>
                    <TabPanel value={Tabvalue} index={1}>
                      <VendorLedger fromDate={fromDate} toDate={toDate} data="payable" />
                    </TabPanel>
                  </div>
                )}
              </Paper>
...
export default InjectObserver(AccountPayableReport);