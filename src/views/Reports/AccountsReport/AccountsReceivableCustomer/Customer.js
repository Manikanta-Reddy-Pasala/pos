...
function ReceivableCustomerList() {
  ...
  const {
    ...
    getAccountsReceivableFromAllTxn,
  ...
  const getCustomerData = async (value) => {
    ...
    await Query.$.subscribe((data) => {
      ...
      if (response.length > 0) {
        ...
        getAccountsReceivableFromAllTxn(firstCustomer);
      } else {
        resetSelectedCustomer();
      }
  ...
  const getDataFromCashFlow = () => {
    ...
    if (accountsReceivableResult && accountsReceivableResult.length > 0) {
      ...
      const totalCashRecord = {
        ...
        'Ref No': '',
        Type: 'Total Payable',
        'To Pay': accountsReceivableTotalToPay
      };
  ...
  async function onRowClicked(row) {
    ...
    await getAccountsReceivableFromAllTxn(row.data);
  }
...
export default InjectObserver(ReceivableCustomerList);
