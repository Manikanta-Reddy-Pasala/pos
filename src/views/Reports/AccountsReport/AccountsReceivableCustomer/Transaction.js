...
const TransactionTable = () => {
  ...
  useEffect(() => {
    if (gridApi) {
      if (accountsReceivableResult) {
        gridApi.setRowData(accountsReceivableResult);
  ...
  const getDataFromCashFlow = () => {
    ...
    if (accountsReceivableResult && accountsReceivableResult.length > 0) {
      for (var i = 0; i < accountsReceivableResult.length; i++) {
        const record = {
          Date: accountsReceivableResult[i].date,
          'Ref No': accountsReceivableResult[i].sequenceNumber,
          Type: accountsReceivableResult[i].txnType,
          Total: accountsReceivableResult[i].amount,
          Balance: accountsReceivableResult[i].balance
        };
        data.push(record);
      }
      const emptyRecord = {};
      ...
      const totalCashRecord = {
        ...
        'Ref No': '',
        Type: 'Total',
        Total: accountsReceivableTotalToPay
      };
    ...
    wb.SheetNames.push('Accounts Receive by Customer');

    console.log('test:: ws::', ws);
    wb.Sheets['Accounts Receive by Customer'] = ws;
    ...
    const fileName = 'Accounts_receivable_by_customer_Sheet';
  ...
  const generatePDFDocument = async () => {
    ...
    saveAs(blob, selectedCustomer.name + '_Transactions');
  };
      ...
      >
        <Grid item xs={10} sm={10}>
          <div className={classes.content}>
            <div className={classes.contentLeft}>
              ...
              <Typography gutterBottom style={{ fontSize: '14px' }}>
                Phone No :{' '}
                {selectedCustomer && selectedCustomer.phoneNo
        ...
        <Grid item xs={12}>
          <div
            ...
            className="red-theme "
            style={{ width: '100%', height: height - 198 + 'px' }}
          >
            <div
              id="product-list-grid"
            ...
            >
              <AgGridReact
                ...
                suppressMenuHide
                rowData={accountsReceivableResult}
