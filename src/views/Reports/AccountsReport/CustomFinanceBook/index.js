...
import CustomFinanceBookPDF from 'src/views/PDF/CustomFinanceBook/CustomFinanceBookPDF';
...
import { downloadExcelFromWorkBookBuffer, prepareCustomFinanceHeaderRow } from 'src/utils/report';
...
const CustomFinanceBook = () => {
  ...
  const {
    customFinanceTotalIn,
    customFinanceTotalOut,
    customFinanceTransactionList
  } = toJS(store.BankAccountsStore);
  const { getCustomFinanceTransactionsByDate } = store.BankAccountsStore;
  ...
  useEffect(() => {
    if (gridApi) {
      if (customFinanceTransactionList) {
        gridApi.setRowData(customFinanceTransactionList);
      }
    }
  }, [customFinanceTransactionList]);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      getCustomFinanceTransactionsByDate(fromDate, toDate, selectedFinance);
    }
  ...
  const getDataXlsx = async() => {
    ...
    const worksheet = workbook.addWorksheet(
      'Custom Finance RECORDS (' + parseInt(customFinanceTransactionList.length || 0) + ')'
    );
    let filteredColumns = [];
    await prepareCustomFinanceHeaderRow(filteredColumns);
    ...
    const emptyRow  = worksheet.addRow({});
      ...
      emptyRow.getCell('financeName').value =""
      emptyRow.getCell('txnType').value =""
      emptyRow.getCell('cashIn').value =""
      emptyRow.getCell('cashOut').value = ""
    console.log(customFinanceTransactionList);
    if (
      customFinanceTransactionList &&
      customFinanceTransactionList.length > 0
    ) {
      for (var i = 0; i < customFinanceTransactionList.length; i++) {
        const newRow  = worksheet.addRow({});
        newRow.getCell('date').value = getDate(customFinanceTransactionList[i])
        newRow.getCell('sequenceNumber').value =  customFinanceTransactionList[i].sequenceNumber
        newRow.getCell('particulars').value = getName(customFinanceTransactionList[i])
        newRow.getCell('txnType').value =customFinanceTransactionList[i].txnType
        newRow.getCell('financeName').value = getFinanceName(customFinanceTransactionList[i])
        newRow.getCell('cashIn').value = getCashIn(customFinanceTransactionList[i])
        newRow.getCell('cashOut').value = getCashOut(customFinanceTransactionList[i])
      ...
      totalCashInRow.getCell('sequenceNumber').value =  customFinanceTotalIn
      totalCashInRow.getCell('financeName').value = ""
      ...
      totalCashOutRow.getCell('sequenceNumber').value =  customFinanceTotalOut
      totalCashOutRow.getCell('financeName').value = ""
    ...
    const fileName = 'Custom_Finance_Report';
  ...
  const generatePDFDocument = async () => {
    const total = {
      cashIn: customFinanceTotalIn,
      cashOut: customFinanceTotalOut,
    };

    const blob = await pdf(
      <CustomFinanceBookPDF
        data={customFinanceTransactionList}
    ...
    saveAs(blob, 'Custom_Finance_Book');
  };
                      ...
                      <div className={classes.selectOption}>
                        ...
                        >
                          <Select
                            ...
                            value={selectedFinance ? selectedFinance : 'Select'}
                            input={<Input />}
                            onChange={async (e) => {
                              if (e.target.value !== 'Select') {
                                setSelectedFinance(e.target.value);
                                getCustomFinanceTransactionsByDate(fromDate, toDate, e.target.value);
                              } else {
                                setSelectedFinance('');
                                getCustomFinanceTransactionsByDate(fromDate, toDate, '');
                              }
                            }}
                          >
                            ...
                            {splitPaymentSettings &&
                              splitPaymentSettings.customFinanceList &&
                              splitPaymentSettings.customFinanceList.map(
                                (option, index) => (
                  ...
                  >
                    ...
                    >
                      <AgGridReact
                        ...
                        suppressMenuHide
                        rowData={customFinanceTransactionList}
...
export default InjectObserver(CustomFinanceBook);