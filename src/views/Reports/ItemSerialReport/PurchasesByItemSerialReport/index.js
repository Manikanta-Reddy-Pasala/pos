...
const PurchasesByItemSerialNo = () => {
  ...
  const handleCellClicked = async (event) => {
    ...
    if ('sequenceNumber' === colId) {
      let data = await getPurchasesDataById(event.data.txnId);
  ...
  const getDataFromPurchasesByItemReport = async () => {
    ...
    await prepareHeaderRow(filteredColumns, 'PURCHASE', 'VENDOR');
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      const fileName = `${localStorage.getItem(
        'businessName'
      )}_Purchase_By_Item_Serial_Report_${convertDateToYYYYMMDD(
        fromDate
...
export default InjectObserver(PurchasesByItemSerialNo);