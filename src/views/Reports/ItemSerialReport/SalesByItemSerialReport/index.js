...
const SalesByItemSerialNo = () => {
  ...
  const getDataFromSalesByItemReport = async () => {
    ...
    await prepareHeaderRow(filteredColumns, 'SALE', 'CUSTOMER');
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      const fileName = `${localStorage.getItem(
        'businessName'
      )}_Sales_By_Item_Serial_Report_${convertDateToYYYYMMDD(
        fromDate
  ...
  async function fetchData() {
    ...
    if (modelNo !== '') {
      ...
      let txnData = await getTxnByAllSerialNo(db, fromDate, toDate, 'Sales');
      setRowData(txnData);
      calculateAggrigatedValues(txnData);
    }
  }
...
export default InjectObserver(SalesByItemSerialNo);