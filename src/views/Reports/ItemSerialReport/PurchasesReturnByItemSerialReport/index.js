...
const PurchasesReturnByItemSerialNo = () => {
  ...
  const { viewOrEditItem } = store.PurchasesReturnsAddStore;
  ...
  const handleCellClicked = async (event) => {
    ...
    if ('sequenceNumber' === colId) {
      let data = await getPurchasesReturnDataById(event.data.txnId);
  ...
  const getSerialData = (data) => {
    let result = parseFloat(data.serialOrImeiNo);
    if (data.serialNo && data.serialNo.length > 0) {
      result = data.serialNo.join(', ');
    }
    return result;
  ...
  const getDataFromPurchasesByItemReport = async () => {
    ...
    await prepareHeaderRow(filteredColumns, 'RETURN', 'VENDOR');
    ...
    for (var i = 0; i < rowData.length; i++) {
      ...
      newRow.getCell('itemSerialNo').value = getSerialData(rowData[i]);
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      const fileName = `${localStorage.getItem(
        'businessName'
      )}_Purchase_Return_By_Item_Serial_Report_${convertDateToYYYYMMDD(
        fromDate
  ...
  const calculateAggrigatedValues = async (txnData) => {
    ...
    txnData.forEach((res) => {
      totalPurchasesAmount += res.amount;
              ...
              <div className={classes.content}>
                <div className={classes.contentLeft}>
                  <Typography gutterBottom variant="h4" component="h4">
                    PURCHASES RETURN BY ITEM SERIAL NO
                  </Typography>
                </div>
              </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <Grid item style={{ marginLeft: '20px' }}>
                        <Typography>
                          Total RETURN Amount : {totalPurchasesAmount}{' '}
                        </Typography>
                      </Grid>
                    </Grid>
                  </Grid>
...
export default InjectObserver(PurchasesReturnByItemSerialNo);