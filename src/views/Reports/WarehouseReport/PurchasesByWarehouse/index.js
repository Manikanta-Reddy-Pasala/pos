...
import {
  ...
  getPurchasesTxn,
  getPurchasesTxnByCategoryAndWarehouse,
  getPurchasesTxnByProductAndWarehouse,
  getPurchasesTxnAndWarehouse
...
const PurchasesByWarehouse = () => {
  ...
  const handleChangeSubCategory = async (event) => {
    ...
    const db = await Db.get();
    let txnData = await getPurchasesTxnByCategory(
      db,
      level2Category.name,
      event.name,
      fromDate,
  ...
  const handleProductClick = async (option) => {
    ...
    if (warehouse !== '') {
      let txnData = await getPurchasesTxnByProductAndWarehouse(
        db,
        option.productId,
        fromDate,
        toDate,
      ...
      let txnData = await getPurchasesTxnByProduct(
        db,
        option.productId,
        fromDate,
        toDate
  ...
  async function fetchData() {
    ...
    if (level2Category.name && level3Category.name) {
      txnData = await getPurchasesTxnByCategory(
        db,
        level2Category.name,
        level3Category.name,
        fromDate,
      ...
      if (selectedProduct && selectedProduct.productId) {
        txnData = await getPurchasesTxnByProduct(
          db,
        ...
        txnData = await getPurchasesTxn(db, fromDate, toDate);
        setRowData(txnData);
      }
    }
    calculateAggrigatedValues(txnData);
  ...
  async function fetchWarehouseData(warehouseSelected) {
    ...
    let txnData;
    if (level2Category.name && level3Category.name) {
      txnData = await getPurchasesTxnByCategoryAndWarehouse(
        db,
        level2Category.name,
        level3Category.name,
        fromDate,
      ...
      if (selectedProduct && selectedProduct.productId) {
        txnData = await getPurchasesTxnByProductAndWarehouse(
          db,
          selectedProduct.productId,
          fromDate,
          toDate,
        ...
        txnData = await getPurchasesTxnAndWarehouse(
          db,
          fromDate,
          toDate,
          warehouseSelected
  ...
  const calculateAggrigatedValues = async (txnData) => {
    ...
    txnData.forEach((res) => {
      totalPurchaseAmount += res.purchaseAmount;
      totalPurchaseQty += res.purchaseQty;
    });

    //set env variables
    setTotalPurchaseAmount(totalPurchaseAmount);
    setTotalPurchaseQty(totalPurchaseQty);
  };
...
export default InjectObserver(PurchasesByWarehouse);
