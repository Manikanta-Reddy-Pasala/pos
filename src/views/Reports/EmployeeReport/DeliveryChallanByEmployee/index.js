...
const DeliveryChallanByEmployee = (props) => {
  ...
  const { viewOrEditDeliveryChallanTxnItem } = stores.DeliveryChallanStore;
  ...
  const handleCellClicked = (event) => {
    ...
    if ('sequenceNumber' === colId) {
      viewOrEditDeliveryChallanTxnItem(event.data);
    }
  };
  ...
  const getDataFromDeliveryChallanByEmployee = async () => {
    const wb = new Workbook();

    let allSalesData = [];
    if (employeeId != null) {
      allSalesData = await getAllDeliveryChallanDataByDateAndEmployeeToDownload(
        fromDate,
        toDate,
        employeeId
      );
    } else {
      allSalesData = await getAllDeliveryChallanDataByDateToDownload(
        fromDate,
        toDate
      );
    }
    ...
    if (allSalesData && allSalesData.length > 0) {
      for (var i = 0; i < allSalesData.length; i++) {
        const record = {
          ...
          'DC NUMBER': allSalesData[i].sequenceNumber,
    ...
    wb.SheetNames.push('Del Challan By Employee Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Del Challan By Employee Sheet'] = ws;
    ...
    const fileName = 'Delivery_Challan_By_Employee_Report';
  ...
  const { handleDeliveryChallanByEmployeeSearch } = stores.DeliveryChallanStore;
  ...
  const handleSearch = async (e) => {
    if (e) {
      ...
      if (target) {
        setSalesData(
          await handleDeliveryChallanByEmployeeSearch(
            target,
            fromDate,
            toDate,
            employeeId
        ...
        if (employeeId) {
          getDeliveryChallanDataByDateAndEmployee(fromDate, toDate, employeeId);
        } else {
          getDeliveryChallanDataByDate(fromDate, toDate);
        }
      }
    }
  };
  ...
  const getDeliveryChallanDataByDate = async (fromDate, toDate) => {
  ...
  const getDeliveryChallanDataByDateAndEmployee = async (
    fromDate,
    toDate,
    employeeId
  ) => {
  ...
  const getAllDeliveryChallanDataByDateToDownload = async (
    fromDate,
    toDate
  ) => {
    const db = await Db.get();
  ...
  const getAllDeliveryChallanDataByDateAndEmployeeToDownload = async (
    fromDate,
    toDate,
    phoneNo
  ) => {
  ...
  const handleEmployeeClick = (employee) => {
    ...
    getDeliveryChallanDataByDateAndEmployee(
      fromDate,
      toDate,
      employee.employeeId
    );
                ...
                >
                  <Grid container direction="row" alignItems="center">
                    ...
                    <Grid xs={2} className="category-actions-right">
                      <Avatar>
                        <IconButton
                          onClick={() => getDataFromDeliveryChallanByEmployee()}
                        >
...
export default InjectObserver(DeliveryChallanByEmployee);
