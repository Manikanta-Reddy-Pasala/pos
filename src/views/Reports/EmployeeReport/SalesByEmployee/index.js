...
const SalesByEmployee = (props) => {
  ...
  const { openAddSalesInvoice, isLaunchEWayAfterSaleCreation, printData } = toJS(stores.SalesAddStore);
  ...
  const getDataFromSalesByEmployee = async () => {
    const wb = new Workbook();

    let allSalesData = [];
    if (employeeId != null) {
      allSalesData = await getAllSaleDataByDateAndEmployeeToDownload(
        fromDate,
        toDate,
        employeeId
      );
    ...
    if (allSalesData && allSalesData.length > 0) {
      for (var i = 0; i < allSalesData.length; i++) {
        const record = {
          ...
          STATUS: getSaleStatus(allSalesData[i])
        };
        data.push(record);
      }
    } else {
    ...
    wb.SheetNames.push('Sales By Employee Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Sales By Employee Sheet'] = ws;
    ...
    const fileName = 'Sales_By_Employee_Report';
  ...
  const handleSearch = async (e) => {
    if (e) {
      ...
      if (target) {
        getSalesSearchWithDate(target, fromDate, toDate, employeeId);
      } else {
        if (employeeId) {
          getSaleDataByDateAndEmployee(fromDate, toDate, employeeId);
        } else {
  ...
  const handleEmployeeClick = (employee) => {
    ...
    getSaleDataByDateAndEmployee(fromDate, toDate, employee.employeeId);
  };
    ...
    const loadPaginationData = async () => {
      if (onChange && fromDate && toDate) {
        ...
        if (searchText && searchText.length > 0) {
          getSalesSearchWithDate(searchText, fromDate, toDate, employeeId);
        } else {
          if (employeeId != null) {
            getSaleDataByDateAndEmployee(fromDate, toDate, employeeId);
          } else {
  ...
  const getSaleDataByDateAndEmployee = async (fromDate, toDate, employeeId) => {
    const db = await Db.get();
    var Query;
  ...
  const getAllSaleDataByDateAndEmployeeToDownload = async (
    fromDate,
    toDate,
    phoneNo
  ) => {
  ...
  const getSalesSearchWithDate = async (
    ...
    if (currentPage && currentPage > 1) {
      ...
      getAllSalesSearchWithDate(value, fromDate, toDate, employeeId);
    }
    const businessData = await Bd.getBusinessData();
        ...
        <div className={classes.root} style={{ minHeight: height - 83 }}>
          {isFeatureAvailable ? (
            <Paper className={classes.root} style={{ minHeight: height - 54 }}>
              ...
              >
                <div>
                  <Typography className={classes.texthead}>Paid</Typography>
                  <Card
                    className={classes.headerRoot}
                    style={{ border: '1px solid #33ff00', borderRadius: 8 }}
                  >
                    <Typography className={classes.text}>{`₹${parseFloat(
                      allSaleData.paid || 0
                    ).toFixed(2)}`}</Typography>
                  </Card>
                </div>

                <Typography variant="h6" className={classes.plus}>
                  +
                </Typography>
                <div>
                  <Typography className={classes.texthead}>Unpaid</Typography>
                  <Card
                    className={classes.headerRoot}
                ...
                <div>
                  <Typography className={classes.texthead}>Total</Typography>
                  <Card
                    className={classes.headerRoot}
                    style={{ border: '1px solid #4a83fb', borderRadius: 8 }}
                  >
                    <Typography className={classes.text}>{`₹${parseFloat(
                      Number(allSaleData.paid) + Number(allSaleData.unPaid) || 0
                    ).toFixed(2)}`}</Typography>
                  </Card>
                </div>
              </Grid>
                ...
                >
                  <Grid container direction="row" alignItems="center">
                    ...
                    <Grid xs={2} className="category-actions-right">
                      <Avatar>
                        <IconButton
                          onClick={() => getDataFromSalesByEmployee()}
                        >
...
export default InjectObserver(SalesByEmployee);