...
const JobWorkInByEmployee = (props) => {
  ...
  const getDataForXls = async () => {
    ...
    if (allSalesData && allSalesData.length > 0) {
      for (var i = 0; i < allSalesData.length; i++) {
        const record = {
          ...
          BALANCE: allSalesData[i].balance_amount,
    ...
    wb.SheetNames.push('JWI By Employee Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['JWI By Employee Sheet'] = ws;
    ...
    const fileName = 'JWI_By_Employee_Report';
  ...
  const handleSearch = async (e) => {
    if (e) {
      ...
      if (target) {
        setRowData(
          await handleJobWorkInSearchWithDateAndEmployee(
            target,
            fromDate,
            toDate,
            employeeId
...
export default InjectObserver(JobWorkInByEmployee);
