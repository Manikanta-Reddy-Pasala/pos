...
const PurchasesByEmployee = (props) => {
  ...
  const getDataFromPurchasesByEmployee = async () => {
    ...
    if (employeeId != null) {
      xlsxPurchaseData = await getAllPurchaseDataByEmployeeXlsx(
        fromDate,
        toDate,
        employeeId
      );
    ...
    if (xlsxPurchaseData && xlsxPurchaseData.length > 0) {
      for (var i = 0; i < xlsxPurchaseData.length; i++) {
        const record = {
          ...
          STATUS: getPurchaseStatus(xlsxPurchaseData[i])
        };
        data.push(record);
      }
    } else {
    ...
    wb.SheetNames.push('Purchases By Employee Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Purchases By Employee Sheet'] = ws;
    ...
    const fileName = 'Purchases_By_Employee_Report';
  ...
  const {
    handlePurchasesSearchByEmployeeAndDate,
    handlePurchasesSearchWithDate
  ...
  const handleSearch = async (e) => {
    if (e) {
      ...
      if (target) {
        if (employeeId) {
          setPurchaseData(
            await handlePurchasesSearchByEmployeeAndDate(
              target,
              fromDate,
              toDate,
              employeeId
          ...
          setPurchaseData(
            await handlePurchasesSearchWithDate(target, fromDate, toDate)
          );
        }
      } else {
        if (employeeId) {
          getPurchaseDataByEmployee(fromDate, toDate, employeeId);
        } else {
  ...
  const handleEmployeeClick = (employee) => {
    ...
    getPurchaseDataByEmployee(fromDate, toDate, employee.employeeId);
  };
  ...
  const getPurchaseDataByEmployee = async (fromDate, toDate, phoneNo) => {
    const db = await Db.get();
    var Query;
    ...
    if (currentPage && currentPage > 1) {
      ...
      getAllPurchaseDataByEmployee(fromDate, toDate, phoneNo);
    }
    const businessData = await Bd.getBusinessData();
  ...
  const getAllPurchaseDataByEmployee = async (fromDate, toDate, phoneNo) => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
  ...
  const getAllPurchaseDataByEmployeeXlsx = async (
    fromDate,
    toDate,
    phoneNo
  ) => {
...
export default InjectObserver(PurchasesByEmployee);
