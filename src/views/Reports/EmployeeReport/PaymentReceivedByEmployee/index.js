...
const PaymentMadebyEmployeeReport = () => {
  ...
  const { viewOrEditPaymentInTxnItem } = stores.PaymentInStore;
  ...
  const handleCellClicked = (event) => {
    ...
    if ('sequenceNumber' === colId) {
      viewOrEditPaymentInTxnItem(event.data);
    }
  };
    ...
    },
    {
      ...
      valueFormatter: (params) => {
        return params['data']['received']
          ? parseFloat(params['data']['received']).toFixed(2)
          : '0';
      }
    },
    {
  ...
  const handleEmployeeClick = (employee) => {
    ...
    getAllPaymentInDataByDateAndEmployee(employee.employeeId);
    getPaymentInDataByDate(employee.employeeId);
  };

  const getPaymentInDataByDate = async (employeeId) => {
  ...
  const getAllPaymentInDataByDate = async (dateRange) => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
  ...
  const getAllPaymentInDataByDateAndEmployee = async (employeeId) => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
  ...
  const getAllPaymentInDataByDateXlsx = async (dateRange) => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
  ...
  const getAllPaymentInDataByDateAndEmployeeXlsx = async (employeeId) => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
  ...
  const getData = async () => {
    ...
    if (employeeId) {
      xlsxPaymentReceiveList = await getAllPaymentInDataByDateAndEmployeeXlsx(
        fromDate,
        toDate,
        employeeId
      );
    } else {
      xlsxPaymentReceiveList = await getAllPaymentInDataByDateXlsx(
        fromDate,
        todayDate
      );
    }
    ...
    wb.SheetNames.push('Payment Received By Employee');

    console.log('test:: ws::', ws);
    wb.Sheets['Payment Received By Employee'] = ws;
    ...
    const fileName = 'Payment_Received_by_Employee_Report';
