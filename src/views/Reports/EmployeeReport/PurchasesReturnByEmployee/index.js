...
const PurchaseReturnByEmployee = (props) => {
  ...
  const {
    handlePurchasesReturnSearchWithDate,
    handlePurchasesReturnSearchWithEmployeeAndDate
  ...
  const getDataFromPurchasesReturnByEmployee = async () => {
    const wb = new Workbook();

    let xlsxReturnData = [];

    if (employeeId != null) {
      xlsxReturnData = await getAllPurchasesReturnListByEmployeeXlsx(
        fromDate,
        toDate,
        employeeId
      );
    ...
    wb.SheetNames.push('Purchases Return Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Purchases Return Sheet'] = ws;
    ...
    const fileName = 'Purchases_Return_by_Employee_Report';
      ...
      if (target) {
        if (employeeId != null) {
          setReturnDetailsList(
            await handlePurchasesReturnSearchWithEmployeeAndDate(
              target,
              fromDate,
              toDate,
              employeeId
        ...
        if (employeeId != null) {
          getPurchasesReturnListByEmployee(fromDate, toDate, employeeId);
        } else {
  ...
  const handleEmployeeClick = (employee) => {
    ...
    getPurchasesReturnListByEmployee(fromDate, toDate, employee.employeeId);
  };
  ...
  const getPurchasesReturnListByEmployee = async (
    fromDate,
    toDate,
    mobileNo
  ) => {
    ...
    if (currentPage && currentPage > 1) {
      ...
      getAllPurchasesReturnListByEmployee(fromDate, toDate, employeeId);
    }
    const businessData = await Bd.getBusinessData();
  ...
  const getAllPurchasesReturnListByEmployee = async (
    fromDate,
    toDate,
    employeeId
  ) => {
  ...
  const getAllPurchasesReturnListByEmployeeXlsx = async (
    fromDate,
    toDate,
    mobileNo
  ) => {
                ...
                >
                  ...
                  >
                    ...
                    <Grid xs={2} className="category-actions-right">
                      <Avatar>
                        <IconButton
                          onClick={() => getDataFromPurchasesReturnByEmployee()}
                        >
...
export default InjectObserver(PurchaseReturnByEmployee);
