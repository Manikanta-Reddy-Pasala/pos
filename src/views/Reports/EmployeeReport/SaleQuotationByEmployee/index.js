...
const SaleQuotationByEmployee = (props) => {
  ...
  const { viewOrEditSaleQuotationItem } = stores.SalesQuotationAddStore;
  ...
  const getDataFromSalesByEmployee = async () => {
    ...
    let allSalesData = [];
    if (employeeId != null) {
      allSalesData = await getAllSaleQuotationDataByDateAndEmployeeToDownload(
        fromDate,
        toDate,
        employeeId
      );
    } else {
      allSalesData = await getAllSaleQuotationDataByDateToDownload(
        fromDate,
        toDate
      );
    }
    ...
    if (allSalesData && allSalesData.length > 0) {
      for (var i = 0; i < allSalesData.length; i++) {
        const record = {
          ...
          'ESTIMATE NO': allSalesData[i].sequenceNumber,
          CUSTOMER: allSalesData[i].customer_name,
          AMOUNT: allSalesData[i].total_amount,
          STATUS: allSalesData[i].estimateType
        };
        data.push(record);
      }
    } else {
    ...
    wb.SheetNames.push('Quotations By Employee Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['Quotations By Employee Sheet'] = ws;
    ...
    const fileName = 'Quotations_By_Employee_Report';
  ...
  const {
    handleSaleQuotationByEmployeeSearch,
    handleSaleQuotationByDateSearch
  ...
  const handleSearch = async (e) => {
    if (e) {
      ...
      if (target) {
        if (employeeId != null) {
          setSalesData(
            await handleSaleQuotationByEmployeeSearch(
              target,
              fromDate,
              toDate,
              employeeId
          ...
          setSalesData(
            await handleSaleQuotationByDateSearch(target, fromDate, toDate)
          );
        }
      } else {
        if (employeeId) {
          getSaleQuotationDataByDateAndEmployee(fromDate, toDate, employeeId);
        } else {
          getSaleQuotationDataByDate(fromDate, toDate);
        }
      }
    }
  };
  ...
  const getSaleQuotationDataByDate = async (fromDate, toDate) => {
  ...
  const getSaleQuotationDataByDateAndEmployee = async (
    fromDate,
    toDate,
    employeeId
  ) => {
  ...
  const getAllSaleQuotationDataByDateToDownload = async (fromDate, toDate) => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
  ...
  const getAllSaleQuotationDataByDateAndEmployeeToDownload = async (
    fromDate,
    toDate,
    phoneNo
  ) => {
  ...
  const handleEmployeeClick = (employee) => {
    ...
    getSaleQuotationDataByDateAndEmployee(
      fromDate,
      toDate,
      employee.employeeId
    );
...
export default InjectObserver(SaleQuotationByEmployee);
