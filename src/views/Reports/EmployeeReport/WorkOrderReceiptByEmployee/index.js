...
const WorkOrderReceiptByEmployee = (props) => {
  ...
  const { viewOrderReceipt } = stores.JobWorkReceiptStore;
  ...
  const handleCellClicked = async (event) => {
    ...
    if ('receiptSequenceNumber' === colId) {
      viewOrderReceipt(event.data);
    }
  };
  ...
  const getDataForXls = async () => {
    ...
    let allSalesData =
      await getAllWorkOrderReceiptDataByDateAndEmployeeToDownload(
        fromDate,
        toDate
      );
    ...
    if (allSalesData && allSalesData.length > 0) {
      for (var i = 0; i < allSalesData.length; i++) {
        const record = {
          DATE: formatDownloadExcelDate(allSalesData[i].receiptDate),
          'RECEIPT NO.': allSalesData[i].receiptSequenceNumber,
          'VENDOR NAME': allSalesData[i].vendorName,
          AMOUNT: allSalesData[i].totalAmount
        };
        data.push(record);
      }
    } else {
    ...
    wb.SheetNames.push('WO By Employee Sheet');

    console.log('test:: ws::', ws);
    wb.Sheets['WO By Employee Sheet'] = ws;
    ...
    const fileName = 'WO_By_Employee_Report';
  ...
  const { handleOrderReceiptSearchByDateAndEmployee } =
  ...
  const handleEmployeeClick = (employee) => {
    ...
    getJobWorkReceiptDataByDate(fromDate, toDate, employee.employeeId);
  };

  const getJobWorkReceiptDataByDate = async (fromDate, toDate, employeeId) => {
  ...
  const getAllWorkOrderReceiptDataByDateAndEmployeeToDownload = async (
    fromDate,
    toDate
  ) => {
    const db = await Db.get();
  ...
  useEffect(() => {
    const loadPaginationData = async () => {
      if (onChange && fromDate && toDate) {
        setOnChange(false);
        setRowData([]);
        getJobWorkReceiptDataByDate(fromDate, toDate);
      }
    };
  ...
  const handleSearch = async (e) => {
    if (e) {
      ...
      if (target) {
        setRowData(
          await handleOrderReceiptSearchByDateAndEmployee(
            target,
            fromDate,
            toDate,
            employeeId
        ...
        getJobWorkReceiptDataByDate(fromDate, toDate, employeeId);
      }
    }
  };
...
export default InjectObserver(WorkOrderReceiptByEmployee);
