...
import SalesAgingPDF from 'src/views/PDF/SalesAging/SalesAgingPDF';
...
const SaleAgingDetails = (props) => {
  ...
  const getExcelDataFromSales = async () => {
    ...
    let allSalesData = await getAllExcelSalesData();
    let data = [];
    ...
    wb.SheetNames.push('Sale Aging Details Sheet');

    // console.log('test:: ws::', ws);
    wb.Sheets['Sale Aging Details Sheet'] = ws;
    ...
    const fileName = 'Sale_Aging_Details_Report';
  ...
  useEffect(() => {
    const loadPaginationData = async () => {
      if (onChange && toDate) {
        setOnChange(false);
        await getSaleDataByDate(toDate);
      }
    };
  ...
  const getSaleDataByDate = async (toDate) => {
  ...
  const getAllExcelSalesData = async () => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
  ...
  const getAllSaleDataByDate = async (toDate) => {
    const db = await Db.get();
    var Query;
  ...
  const generatePDFDocument = async () => {
    ...
    saveAs(blob, 'Sale_Aging_Details');
  };
              ...
              <div className={classes.content}>
                <div className={classes.contentLeft}>
                  <Typography gutterBottom variant="h4" component="h4">
                    SALE AGING DETAILS BY INVOICE DUE DATE
                  </Typography>
                </div>
              </div>
...
export default InjectObserver(SaleAgingDetails);