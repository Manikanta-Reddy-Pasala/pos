...
const CustomerSaleAgingReceivableSummary = () => {
  ...
  const getExcelDataDownload = async () => {
    ...
    wb.SheetNames.push('Customer Receivable Sheet');

    // console.log('test:: ws::', ws);
    wb.Sheets['Customer Receivable Sheet'] = ws;
    ...
    const fileName = 'Customer_Sale_Aging_Receivable_Report';
  ...
  const handleCustomerClick = async (customer) => {
    ...
    //set env variables
    setCurrentAmount(0);
    setOneToFifteenAmount(0);
    setSixteenToThirtyAmount(0);
    setThirtyoneToFortyfiveAmount(0);
    setFortysixTosixtyAmount(0);
    setOverSixtyAmount(0);
  ...
  const generatePDFDocument = async () => {
    ...
    const blob = await pdf(
      <CustomerAccountingPDF
        ...
        total={total}
        type={'Receivable'}
        title={'Customer Accounts Receivable (Aging)'}
      />
    ).toBlob();

    console.log(blob);

    saveAs(blob, 'Customer_Accounts_Receivable_(Aging)');
  };
...
export default InjectObserver(CustomerSaleAgingReceivableSummary);