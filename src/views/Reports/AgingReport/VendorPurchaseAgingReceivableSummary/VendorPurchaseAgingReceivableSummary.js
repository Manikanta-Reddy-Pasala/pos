...
import VendorAccountingPDF from 'src/views/PDF/VendorAccountAging/VendorAccountingPDF';
...
const VendorPurchaseAgingReceivableSummary = () => {
  ...
  const [vendorPhoneNo, setVendorPhoneNo] = React.useState('');
    ...
    return (
      <Button
        ...
        onClick={() => {
          ...
          setSalesData([]);
          getAllInvoicesData(data.vendor_phone_no);
        }}
      >
        View
      </Button>
  ...
  const getExcelDataDownload = async () => {
    ...
    if (salesData && salesData.length > 0) {
      for (var i = 0; i < salesData.length; i++) {
        const record = {
          NAME: salesData[i].vendor_name,
          'PHONE NUMBER': salesData[i].vendor_phone_no,
          ...
          'TOTAL RECEIVABLE': salesData[i].totalReceivable
        };
        data.push(record);
      }
    } else {
    ...
    wb.SheetNames.push('Vendor Receivable Sheet');

    // console.log('test:: ws::', ws);
    wb.Sheets['Vendor Receivable Sheet'] = ws;
    ...
    const fileName = 'Vendor_Sale_Aging_Receivable_Report';
  ...
  const getVendorData = async (vendorPhNo) => {
    ...
    if (vendorPhNo) {
      const filter = { phoneNo: { $eq: vendorPhNo } };
    ...
    await Query.$.subscribe(async (data) => {
      ...
      if (response.length > 0) {
        for (let vendor of response) {
          await getAccountsReceivableFromAllTxn(vendor, agingDataList);
        }
      }
  ...
  const getAccountsReceivableFromAllTxn = async (vendor, agingDataList) => {
    const db = await Db.get();
    const businessData = await Bd.getBusinessData();
      ...
      .then((data) => {
        ...
        for (let item of finalData) {
          ...
          for (let index = 0; index < agingDataList.length; index++) {
            if (agingDataList[index].vendor_phone_no === vendor.phoneNo) {
              searchIndex = index;
              break;
            }
          }
          ...
          if (item.dueDate !== null && item.dueDate !== undefined) {
            ...
            agingData.vendor_name = vendor.name;
            agingData.vendor_phone_no = vendor.phoneNo;
            agingData.totalReceivable += item.balance;
  ...
  const handleVendorClick = async (customer) => {
    setVendorName(customer.name);
    setVendorPhoneNo(customer.phoneNo);

    getVendorData(customer.id);
    setVendorList([]);
  };
  ...
  const getAllInvoicesData = async (vendorPhNo) => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
    ...
    let filterArray = [
      ...
      {
        vendor_phone_number: { $eq: vendorPhNo }
      },
      {
        updatedAt: { $exists: true }
      }
  ...
  const generatePDFDocument = async () => {
    ...
    salesData.forEach((res) => {
      ...
      total.totalReceivable += res.totalReceivable;
    });
    ...
    const blob = await pdf(
      <VendorAccountingPDF
        ...
        total={total}
        type={'Receivable'}
        title={'Vendor Accounts Receivable (Aging)'}
      />
    ).toBlob();

    console.log(blob);

    saveAs(blob, 'Vendor_Accounts_Receivable_(Aging)');
  };
              ...
              <div className={classes.content}>
                <div className={classes.contentLeft}>
                  <Typography gutterBottom variant="h4" component="h4">
                    VENDOR ACCOUNTS RECEIVABLE (AGING)
                  </Typography>
                </div>
              </div>
...
export default InjectObserver(VendorPurchaseAgingReceivableSummary);