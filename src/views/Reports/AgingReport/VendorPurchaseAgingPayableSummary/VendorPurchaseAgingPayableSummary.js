...
const VendorPurchaseAgingPayableSummary = () => {
  ...
  const getExcelDataDownload = async () => {
    ...
    wb.SheetNames.push('Vendor Payable Sheet');

    // console.log('test:: ws::', ws);
    wb.Sheets['Vendor Payable Sheet'] = ws;
    ...
    const fileName = 'Vendor_Sale_Aging_Payable_Report';
  ...
  const generatePDFDocument = async () => {
    ...
    const blob = await pdf(
      <VendorAccountingPDF
        ...
        total={total}
        type={'Payable'}
        title={'Vendor Accounts Payable (Aging)'}
      />
    ).toBlob();

    console.log(blob);

    saveAs(blob, 'Vendor_Accounts_Payable_(Aging)');
  };
...
export default InjectObserver(VendorPurchaseAgingPayableSummary);