...
const SplitPaymentDetails = (props) => {
  ...
  const saveSplitPaymentDetails = async () => {
    if (getTotalAmount() !== orderData.total_amount) {
      setErrorAlertProductMessage(
        'Please check the total amount provided. Its not matching the KOT bill total amount.'
      );
  ...
  const getTotalAmountGiven = () => {
    let result = 0;
    if (
      orderData.splitPaymentList &&
      orderData.splitPaymentList.length > 0
    ) {
      for (let payment of orderData.splitPaymentList) {
        if (payment.amount !== '') {
    ...
    return orderData.total_amount - result;
  };
      ...
      <DialogContent>
        <div>
          <div>
            {orderData.splitPaymentList &&
              orderData.splitPaymentList.length > 0 &&
              orderData.splitPaymentList.map((option, index) => (
                <div>
                  <div
                    style={{
                      width: '100%',
