...
const Sales = ({ className, labelText, ...rest }) => {
  ...
  const getDailyData = async (fromDate, toDate) => {
    ...
    await query.exec().then(async (data) => {
      ...
      let response = data.map((item) => {
        let output = {};
        output.invoice_date = item.invoice_date;
      ...
      await response.reduce(function (res, value) {
        if (!res[value.invoice_date]) {
          res[value.invoice_date] = {
            invoice_date: value.invoice_date,
            total: 0
          };
          result.push(res[value.invoice_date]);
        }
        res[value.invoice_date].total += value.total_amount;
        return res;
      }, {});
      ...
      result.forEach((element) => {
        let shortMonth = new Date(element.invoice_date).toLocaleString(
          'en-us',
          {
            month: 'short'
          }
        );
        let shortDate = new Date(element.invoice_date).toLocaleString('en-us', {
          day: 'numeric'
        });
  ...
  const getWeeklyData = async (fromDate, toDate) => {
    ...
    await query.exec().then(async (data) => {
      ...
      var result = [];
      await response.reduce(function (res, value) {
        let weekNumber = getWeekOfMonth(new Date(value.invoice_date));
        let monthShort = new Date(value.invoice_date).toLocaleString('en-us', {
          month: 'short'
        });
        ...
        res[date].total += value.total_amount;
        return res;
      }, {});
  ...
  const getMonthlyData = async (fromDate, toDate) => {
    ...
    await query
      ...
      .then(async (data) => {
        ...
        var result = [];
        await response.reduce(function (res, value) {
          let monthShort = new Date(value.invoice_date).toLocaleString(
            'en-us',
            {
              month: 'short'
            }
          ...
          let yearShort = new Date(value.invoice_date).toLocaleString('en-us', {
            year: '2-digit'
          });
        ...
        for (var i = 6; i >= 0; i--) {
          ...
          // console.log('shortMonth::::::', shortMonth + ' ' + shortYear);
