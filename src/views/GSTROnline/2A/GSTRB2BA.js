...
const GSTRB2BA = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const theme = useTheme();
  const [b2b, setB2B] = useState([]);
  const [loading, setLoading] = useState(false);
  const { b2ba2AData, updateB2BAData } = stores.GSTR2AStore;
  ...
  const fetchData = async () => {
    ...
    const apiResponse = await get2AData(reqData,'b2ba');
    setLoading(false);
    if (apiResponse && apiResponse.status === 1) {
      updateB2BAData(apiResponse.message?.b2ba);
    }
  };
      ...
      <table className={`${classes.batchTable}`} style={{ fontSize: '12px' }}>
        <thead>
          <tr>
            ...
            <th rowspan="2" style={{ textAlign: 'center' }} className={`${classes.headstyle} ${classes.rowstyle}`}>
              Supplier Filing Status
            </th>
            <th rowspan="2" className={`${classes.headstyle} ${classes.rowstyle}`}>
              Supplier GSTR-3B Filing Status
            </th>
            <th rowspan="2" className={`${classes.headstyle} ${classes.rowstyle}`}>
              Supplier GSTR-1 Filing Period
            </th>
            <th rowspan="2" className={`${classes.headstyle} ${classes.rowstyle}`}>
              Supplier GSTR-1 Filing Date
            </th>
            <th rowspan="2" className={`${classes.headstyle} ${classes.rowstyle}`}>
              Supplier GSTIN
            </th>
              ...
              {item?.inv?.map((itemb, indexb) => (
                <React.Fragment key={indexb}>
                  <tr>
                    ...
                    <td className={`${classes.rowstyle}`}>
                      <p style={{ color: 'blue' }} onClick={() => handleViewClick(itemb.inum)}>View</p>
                    </td>
                    <td className={`${classes.rowstyle}`}>
                  ...
                  <tr>
                    <td colSpan="15">
                      {openIndex === itemb.inum && (
                        <div >
                          {itemb.itms.map((itemi, i) => (<Grid className={classes.invGrid} container spacing={2}>
                            ...
                            <Grid
                              ...
                              className={`grid-padding ${classes.mb_20} ${classes.pointer}`}
                            >
                              IGST Amount as per invoice : {itemi.itm_det?.iamt}
                            </Grid>
                            <Grid
                              item
                              xs={3}
                              className={`grid-padding ${classes.mb_20} ${classes.pointer}`}
                            >
                              CGST Amount as per invoice : {itemi.itm_det?.camt}
                            </Grid>
                            <Grid
                              item
                              xs={3}
                              className={`grid-padding ${classes.mb_20} ${classes.pointer}`}
                            >
                              SGST Amount as per invoice : {itemi.itm_det?.samt}
                            </Grid>
                            <Grid
                              item
                              xs={3}
                              className={`grid-padding ${classes.mb_20} ${classes.pointer}`}
                            >
                              CESS Amount as per invoice : {itemi.itm_det?.csamt}
                            </Grid>
                          </Grid>))}
                        </div>
...
export default injectWithObserver(GSTRB2BA);