...
const GSTRIMPG = (props) => {
  ...
  const { impg2AData, updateIMPGData } = stores.GSTR2AStore;
  ...
  const fetchData = async () => {
    ...
    const apiResponse = await get2AData(reqData, 'impg');
    setLoading(false);
    if (apiResponse && apiResponse.status === 1) {
      updateIMPGData(apiResponse.message?.impg);
    }
  };
