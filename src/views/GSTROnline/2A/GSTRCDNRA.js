...
const GSTRCDNRA = (props) => {
  ...
  const { cdnra2AData, updateCDNRAData } = stores.GSTR2AStore;
  ...
  const fetchData = async () => {
    ...
    const apiResponse = await get2AData(reqData, 'cdna');
    setLoading(false);
    if (apiResponse && apiResponse.status === 1) {
      updateCDNRAData(apiResponse.message?.cdna);
    }
  };
              ...
              {item?.nt?.map((itemb, indexb) => (
                <React.Fragment key={indexb}>
                  <tr>
                    ...
                    <td className={`${classes.rowstyle}`}>{itemb.ont_num}</td>
                    <td className={`${classes.rowstyle}`}>{itemb.ont_dt}</td>
                    <td className={`${classes.rowstyle}`}></td>
...
export default injectWithObserver(GSTRCDNRA);
