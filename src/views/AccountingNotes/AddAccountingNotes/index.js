...
import MoreOptionsAccountingNotes from 'src/components/MoreOptionsAccountingNotes';
import AddAccountingNotes from '../modal/AddAccountingNotes';
...
const AddAccountingNotesList = (props) => {
  ...
  let [accountingNotes, setAccountingNotesData] = useState([]);
  let [onChange, setOnChange] = useState(true);

  const { accountingNotesDialogOpen, isAccountingNotesList } = toJS(
    stores.AccountingNotesStore
  );

  const { handleAccountingNotesModalOpen } = stores.AccountingNotesStore;
  ...
  const getAccountingNotesDataByDate = async (dateRange) => {
    const db = await Db.get();
    var Query;

    let skip = 0;
    setRowData([]);
    setAccountingNotesData([]);
    if (currentPage && currentPage > 1) {
      skip = (currentPage - 1) * limit;
    } else if (currentPage === 1) {
      getAllAccountingNotesDataByDate(dateRange);
    }
    const businessData = await Bd.getBusinessData();

    Query = db.accountingnotes.find({
      selector: {
        $and: [
    ...
    await Query.$.subscribe((data) => {
      ...
      setAccountingNotesData(results);
    });
  };

  const getAllAccountingNotesDataByDate = async (dateRange) => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
  ...
  const TemplateActionRenderer = (props) => {
    return (
      <MoreOptionsAccountingNotes
        menu={moreoption.moreoptionsdata}
        index={props['data']['id']}
        item={props['data']}
        id={props['data']['id']}
        operationType={'Add Notes'}
        component="notesList"
      />
    );
  };
  ...
  useEffect(() => {
    if (gridApi) {
      if (accountingNotes) {
        gridApi.setRowData(accountingNotes);
      }
    }
  }, [accountingNotes]);
  ...
  useEffect(() => {
    const loadPaginationData = async () => {
      if (onChange) {
        setOnChange(false);
        setRowData([]);
        await getAccountingNotesDataByDate(dateRange);
      }
    };
  ...
  useEffect(() => {
    ...
    loadPaginationData();
  }, [isAccountingNotesList]);

  return (
    <div>
      <Page className={classes.root} title="Accounting Notes">
        {/* <PageHeader /> */}
        ...
        <Paper className={classes.paperRoot}>
          ...
          >
            ...
            <Grid item xs={12} sm={5} align="right">
              <Grid container direction="row" spacing={2} alignItems="center">
                <Grid item xs={7} align="right"></Grid>
          ...
          {accountingNotesDialogOpen ? <AddAccountingNotes /> : null}
        </Paper>
      </Page>
    </div>
  );
};
export default InjectObserver(AddAccountingNotesList);