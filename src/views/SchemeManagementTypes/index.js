...
import AddSchemeManagementTypes from './AddSchemeManagementTypes';
...
const SchemeManagementTypes = () => {
  const classes = useStyles();
  const store = useStore();

  const [isFeatureAvailable, setFeatureAvailable] = useState(true);
  const [isLoading, setLoadingShown] = useState(true);
  const { handleSchemeTypesModalOpen, deleteSchemeTypesData, getSchemeTypes } =
    store.SchemeTypesStore;
  const { schemeTypesDialogOpen, schemeTypesList } = toJS(store.SchemeTypesStore);
  ...
  return (
    <Paper className={classes.root} title="Scheme Types">
        ...
        <div>
          {isFeatureAvailable ? (
            <div>
              ...
              <Button
                onClick={handleSchemeTypesModalOpen}
                style={{
      ...
      {schemeTypesDialogOpen ? <AddSchemeManagementTypes /> : null}
    </Paper>
  );
};

export default InjectObserver(SchemeManagementTypes);
