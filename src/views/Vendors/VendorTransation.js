...
const VendorTransactionTable = (props) => {
  ...
  const { vendor, vendorTransactionList, selectedParty } = toJS(
    store.VendorStore
  );
  const { vendorFromDate, vendorToDate } = toJS(store.VendorStore);
  const { setVendorFromDate, setVendorToDate } = store.VendorStore;
