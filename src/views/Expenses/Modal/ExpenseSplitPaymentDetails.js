...
const ExpenseSplitPaymentDetails = (props) => {
  ...
  const saveSplitPaymentDetails = async () => {
    if (getTotalAmount() !== expense.total) {
      setErrorAlertProductMessage(
        'Please check the total amount provided. Its not matching the bill total amount.'
      );
  ...
  const getTotalAmountGiven = () => {
    ...
    let amount = expense.total - result;
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <div>
          <div>
            {expense && expense.splitPaymentList &&
              expense.splitPaymentList.length > 0 &&
              expense.splitPaymentList.map((option, index) => (
                <div>
                  <div
                    style={{
                      width: '100%',
      ...
      <DialogActions>
        <div>
          <b style={{ marginRight: '20px' }}>TOTAL: {expense.total}</b>
...
export default ExpenseSplitPaymentDetails;
