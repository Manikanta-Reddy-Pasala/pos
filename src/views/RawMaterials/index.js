...
import { getLevel2CategoryByName } from 'src/components/Helpers/BusinessCategoriesQueryHelper';
...
const RawMaterialList = () => {
  ...
  const {
    ...
    handleAddProductModalOpenFromRawMaterials
  } = stores.ProductStore;
  ...
  const TemplateMfgRenderer = (props) => {
    ...
    return String(localStorage.getItem('isHotelOrRestaurant')).toLowerCase() ===
      ...
      <Avatar style={{ height: '25px', width: '25px', marginTop: '10px' }}>
        <IconButton
          style={{
            fontSize: '12px',
            width: '25px',
  ...
  return (
    <div>
      <Page className={classes.root} title="Raw Materials">
        <div>
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            <Grid item xs={6} style={{ textAlign: 'end', marginTop: '20px' }}>
                              <Controls.Button
                                ...
                                onClick={async () => {
                                  let level2 = await getLevel2CategoryByName(
                                    'raw_materials_level2'
                                  );
                                  if (level2) {
                                    handleAddProductModalOpenFromRawMaterials();
                                  } else {
                                    setErrorMessage(
                                      'Raw material category is not added. Please enable category from Business profile to proceed further. Contact customer support for assistance!'
                                    );
                                    setErrorAlertMessage(true);
                                  }
                                }}
...
export default InjectObserver(RawMaterialList);