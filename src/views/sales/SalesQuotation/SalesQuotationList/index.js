...
import MoreOptionsSalesQuotation from '../../../../components/MoreOptionsSalesQuotation';
...
const SalesQuotationList = (props) => {
  ...
  const {
    ...
    printSaleQuotationData,
    openSaleQuotationPrintSelectionAlert
  } = toJS(stores.SalesQuotationAddStore);
  ...
  const { openForNewSale, handleCloseSaleQuotationPrintSelectionAlertMessage } =
  ...
  const TemplateActionRenderer = (props) => {
    return (<MoreOptionsSalesQuotation
  ...
  const getSalesQuotationData = async () => {
    if (saleData) {
  ...
  const closeSaleQuotationDialog = () => {
    handleCloseSaleQuotationPrintSelectionAlertMessage();
  };

  return (
    <div>
      <Page className={classes.root} title="Sales Quotation">
        {/* <PageHeader /> */}
      ...
      {openSaleQuotationPrintSelectionAlert === true ? (
        <OpenPreview
          open={true}
          onClose={closeSaleQuotationDialog}
          id={printSaleQuotationData.sequenceNumber}
          invoiceData={printSaleQuotationData}
          startPrint={false}
        />
      ) : (
        ''
...
export default InjectObserver(SalesQuotationList);