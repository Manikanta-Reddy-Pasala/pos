...
import Moreoptions from '../../../../components/MoreoptionsPaymentIn';
...
const PaymentInList = (props) => {
  ...
  const {
    openForNewPaymnetIn,
    handleClosePaymentInPrintSelectionAlertMessage
  ...
  const {
    openPaymentInPrintSelectionAlert,
    printPaymentInData,
    OpenAddPaymentIn
    ...
    const loadPaginationData = async () => {
      if (onChange) {
        ...
        if (searchText.length > 0) {
          ...
          await getPaymentInDataByDate(dateRange);
        }
      }
    };
  ...
  const getPaymentInDataByDate = async (dateRange) => {
    ...
    if (currentPage && currentPage > 1) {
      ...
      getAllPaymentInDataByDate(dateRange);
    }
  ...
  const handleSearch = async (e) => {
    if (e) {
      ...
      if (target) {
        getPaymentInDataBySearch(target);
      } else {
        getPaymentInDataByDate(dateRange);
      }
    } else {
      getPaymentInDataByDate(dateRange);
    }
  ...
  const getPaymentInDataBySearch = async (value) => {
    const db = await Db.get();
    const businessData = await Bd.getBusinessData();
    var Query;
    ...
    if (currentPage && currentPage > 1) {
      ...
      getAllPaymentInDataBySearch(value);
    }
  ...
  const getAllPaymentInDataBySearch = async (value) => {
    const db = await Db.get();
    const businessData = await Bd.getBusinessData();
    var Query;
    var regexp = new RegExp('^.*' + value + '.*$', 'i');
  ...
  return (
    <Page className={classes.root} title="Payment In">
      <Paper className={classes.pageContent}>
          ...
          <Grid item xs={12} sm={8} align="right">
            <Grid container direction="row" spacing={2} alignItems="center">
              ...
              <Grid item xs={4} align="right">
                ...
                />
                {OpenAddPaymentIn === true ? <AddnewPayment /> : null}
              </Grid>
            </Grid>
          </Grid>
        </Grid>
        ...
        >
          ...
          {openPaymentInPrintSelectionAlert === true ? (
            <OpenPreview
              open={true}
              onClose={closeDialog}
              id={printPaymentInData.receiptNumber}
              invoiceData={printPaymentInData}
              startPrint={false}
            />
          ) : (
            ''
...
export default InjectObserver(PaymentInList);