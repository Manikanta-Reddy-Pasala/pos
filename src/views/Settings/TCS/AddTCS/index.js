...
const AddTCS = (props) => {
  ...
  const { tcs, tcsDialogOpen, isEdit } = toJS(store.TCSStore);
  const {
    handleTCSModalClose,
    setTCSName,
    setTCSRate,
  ...
  return (
    <div>
      <Dialog
        ...
        open={tcsDialogOpen}
        onClose={handleTCSModalClose}
        maxWidth={maxWidth}
      >
        <DialogTitle id="product-modal-title">
          {isEdit ? 'Edit TCS' : 'Add TCS'}
          <IconButton
            aria-label="close"
            onClick={handleTCSModalClose}
          ...
          <Grid container direction="row" alignItems="stretch">
            <Grid item xs={12} className="grid-padding">
              <FormControl fullWidth>
                <Typography variant="subtitle1">TCS Code</Typography>
                <TextField
                  fullWidth
                  required
                  variant="outlined"
                  ...
                  value={tcs.code}
                  onChange={(event) =>
                    setTCSCode(event.target.value.toString())
                  }
                />
              </FormControl>
            </Grid>
            <Grid item xs={12} className="grid-padding">
              <FormControl fullWidth>
                <Typography variant="subtitle1">TCS Name</Typography>
                <TextField
                  fullWidth
                  required
                  variant="outlined"
                  ...
                  value={tcs.name}
                  onChange={(event) =>
                    setTCSName(event.target.value.toString())
                  }
                />
              </FormControl>
            </Grid>
            ...
            >
              <FormControl fullWidth>
                <Typography variant="subtitle1">TCS Rate(%)</Typography>
                <TextField
                  fullWidth
                  variant="outlined"
                  margin="dense"
                  ...
                  value={tcs.rate}
                  onFocus={(e) => (tcs.rate === 0 ? setTCSRate('') : '')}
                  onChange={(event) => {
                    setTCSRate(event.target.value);
                  }}
                  InputProps={{
        ...
        <DialogActions>
          <Button
            ...
            onClick={() => {
              if (tcs.name === '') {
                setErrorMessage('TCS name cannot be left blank');
                setOpenErrorMesssageDialog(true);
              } else if (tcs.rate === '' || tcs.rate === 0) {
                setErrorMessage('Please provide a valid TCS rate');
                setOpenErrorMesssageDialog(true);
              } else {
                if (isEdit) {
                  updateData();
...
export default InjectObserver(AddTCS);