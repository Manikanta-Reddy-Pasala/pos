...
const CustomerLedger = (props) => {
  ...
  useEffect(() => {
    ...
    if (invoiceRegular.strqrcode && invoiceRegular.strqrcode != '' && invoiceRegular.boolQrCode) {
      qrcodeHelper
  ...
  const getDataForExcel = () => {
    ...
    if (
      ...
      for (var i = 0; i < customerLedgerTransactionList.length; i++) {
        let record = {};
        record['DATE'] = getDateFormat(customerLedgerTransactionList[i]);
        record['PARTICULARS'] = getParticulars(
          customerLedgerTransactionList[i]
        );
        record['VCH TYPE'] = customerLedgerTransactionList[i].voucherType;
        record['VCH NO.'] = customerLedgerTransactionList[i].sequenceNumber;
        record['DEBIT'] = customerLedgerTransactionList[i].debit;
        record['CREDIT'] = customerLedgerTransactionList[i].credit;
        if (enableBalance) {
          record['BALANCE'] = customerLedgerTransactionList[i].runningBalance;
        }
  ...
  const generatePDFDocument = async (isUpload, type) => {
    ...
    if (isUpload) {
      handleClose();
      const pdfFile = new File([blob], `${customer.name}_Ledger.pdf`, {
        type: 'application/pdf'
      });
      if (type == 1) {
        shareOnWhatsapp(pdfFile, customer.name + '_Ledger.pdf');
      } else {
        shareOnEmail(pdfFile, customer.name + '_Ledger.pdf');
      }
    } else {
  ...
  const handleSendPDFViaWhatsApp = async (fileUrl) => {
    ...
    const pdfSendReq = {
      ...
      ledgerFileName: customer.name + '_Ledger.pdf'
    };
  ...
  const handleSendPDFViaEmail = async (fileUrl, email) => {
    ...
    const pdfSendReq = {
      ...
      businessCity: businessCity,
      pdfUrl: fileUrl,
      email: customer.emailId !== '' ? customer.emailId : email,
      customerName: customer.name,
      ledgerBalance: getPendingBalanceToReceiveOrPay(),
      ledgerDate: formatDateForLedger(toDate),
      ledgerFileName: customer.name + '_Ledger.pdf'
  ...
  const updateEmailID = () => {
    if (emailId) {
      setIsUpdate();
      setCustomerProperty('emailId', emailId);
      setEmailEmpty(false);
      saveData(false);
