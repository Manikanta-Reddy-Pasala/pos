...
import './CustomerExcelUpload.css';
import { useStore } from '../../../Mobx/Helpers/UseStore';
import InjectObserver from '../../../Mobx/Helpers/injectWithObserver';
import MuiDialogActions from '@material-ui/core/DialogActions';
import exportCustomerDataToExcelAdvanced from './exportCustomerDataToExcelAdvanced';
import exportCustomerTemplateToExcelAdvanced from './exportCustomerTemplateToExcelAdvanced';
...
const DashboardCustomerExcelUpload = () => {
  const classes = useStyles();
  const stores = useStore();

  const {
    saveExcelCustomerRowData,
    updateExcelCustomerRowData,
    addCustomersNotProcessed,
    resetCustomersNotProcessed
  } = stores.CustomerStore;
  const { customersNotProcessed } = toJS(stores.CustomerStore);
  ...
  const readExcelFile = async (uploadedFiles) => {
    ...
    await resetCustomersNotProcessed();
          ...
          await worksheet.eachRow(async (row, rowNumber) => {
            if (rowNumber !== 1) {
              if (isEdit) {
                ...
                await updateExcelCustomerRowData(row, cellValue);
              } else {
                // Get the value of the specified cell in the worksheet
                const customerGSTNCell = worksheet.getCell('D' + rowNumber); // Product name cell address
                const customerGSTNCellValue = customerGSTNCell
                  ? customerGSTNCell.value
                  : '';

                if (customerGSTNCellValue !== '' && customerGSTNCellValue !== null) {
                  const customerData = await findParty({
                    $and: [
                      { gstNumber: { $eq: customerGSTNCellValue } },
                      { isCustomer: true }
                    ]
                  });
                  ...
                  if (customerData) {
                    ...
                    const pnp = {
                      name: customerGSTNCellValue,
                      reason: 'Customer with same GSTIN already exists'
                    };
                    addCustomersNotProcessed(pnp);
                  } else {
                    await saveExcelCustomerRowData(row);
                  }
                } else {
                  await saveExcelCustomerRowData(row);
                }
  ...
  const downloadTemplate = async () => {
    exportCustomerTemplateToExcelAdvanced();
  };

  const downloadDataForEdit = async () => {
    exportCustomerDataToExcelAdvanced();
  };
  ...
  return (
    <div>
    <Page className={classes.root} title="Import Customers Excel">
          ...
          {isFeatureAvailable ? (
            <Container maxWidth={true}>
              <Paper style={{ width: '100%', height: '85vh' }}>
                ...
                {customersNotProcessed && customersNotProcessed.length > 0 && (
                    <div style={{ margin: '20px' }}>
                      <Typography className={classes.header} variant="inherit">
                        List of Customers not processed with errors
                      </Typography>
                      {customersNotProcessed.map((option, index) => (
                        <Typography>
...
export default InjectObserver(DashboardCustomerExcelUpload); 