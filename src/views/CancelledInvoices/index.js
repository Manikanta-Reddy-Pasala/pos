...
import MoreOptionsCancelledData from 'src/components/MoreOptionsCancelledData';
...
const CancelledInvoices = (props) => {
  ...
  let [cancelledData, setCancelledData] = useState([]);
  ...
  const getCancelledDataByDate = async (dateRange) => {
    ...
    if (currentPage && currentPage > 1) {
      ...
      getAllCancelledDataByDate(dateRange);
    }
    const businessData = await Bd.getBusinessData();
    ...
    await Query.$.subscribe((data) => {
      ...
      setCancelledData(response);
    });
  };

  const getAllCancelledDataByDate = async (dateRange) => {
    const db = await Db.get();
    var Query;
    const businessData = await Bd.getBusinessData();
  ...
  function cancelledDateFormatter(params) {
    var dateAsString = params.data.cancelledDate;
    var dateParts = dateAsString.split('-');
    return `${dateParts[2]}-${dateParts[1]}-${dateParts[0]}`;
  }
  ...
  const [columnDefs] = useState([
    ...
    },
    {
      ...
      filter: 'agDateColumnFilter',
      valueFormatter: cancelledDateFormatter,
      headerComponentFramework: (props) => {
        return (
          <div>
            <p>CANCELLED</p>
    ...
    },
    {
      ...
      valueFormatter: (params) => {
        ...
        var timefromTimeStamp = new Date(data.updatedAt).toLocaleTimeString(
          [],
          {
            hour: '2-digit',
            minute: '2-digit',
  ...
  useEffect(() => {
    if (gridApi) {
      if (cancelledData) {
        gridApi.setRowData(cancelledData);
      }
    }
  }, [cancelledData]);
  ...
  useEffect(() => {
    const loadPaginationData = async () => {
      if (onChange) {
        setOnChange(false);
        setRowData([]);
        await getCancelledDataByDate(dateRange);
      }
    };
  ...
  const getAllCancelledDataByDateExcel = async (dateRange) => {
    const db = await Db.get();
    var Query;
  ...
  const getAllExcelCancelledDataByDate = async () => {
    const wb = new Workbook();

    let allData = await getAllCancelledDataByDateExcel(dateRange);
                ...
                <MenuItem value={'Payment Out'}>Payment Out</MenuItem> */}
                    ...
                    <Grid item xs={1}>
                      ...
                      >
                        <Avatar>
                          <IconButton
                            onClick={() => getAllExcelCancelledDataByDate()}
                          >
...
export default InjectObserver(CancelledInvoices);