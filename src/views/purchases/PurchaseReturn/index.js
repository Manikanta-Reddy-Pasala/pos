...
import EmptyPurchaseReturn from './EmptyPurchaseReturn';
...
function SalesReturnCredit() {
  ...
  const { getPurchaseReturnsCount } = store.PurchasesReturnsAddStore;
  const { isPurchaseReturnList } = toJS(store.PurchasesReturnsAddStore);
    ...
    <ThemeProvider theme={theme}>
      ...
      {isLoading && (
          ...
          {!isLoading && (
            <div>
              {isFeatureAvailable ? 
                isPurchaseReturnList ? ( <PurchaseReturnList /> ) : (<EmptyPurchaseReturn />)
                :
                <NoPermission />
              }
         </div>
