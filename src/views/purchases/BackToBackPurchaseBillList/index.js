...
import Moreoptions from 'src/components/MoreOptionsBackToBackPurchase';
...
const BackToBackPurchaseBillList = (props) => {
  ...
  const {
    ...
    openBackToBackPurchasePrintSelectionAlert,
    printBackToBackPurchaseData
  } = toJS(stores.BackToBackPurchaseStore);
  ...
  const { handleCloseBackToBackPrintSelectionAlertMessage } =
  ...
  const TemplateActionRenderer = (props) => {
    return (
      <Moreoptions
        ...
        index={props['data']['backToBackPurchaseNumber']}
        item={props['data']}
        id={props['data']['backToBackPurchaseNumber']}
        component="billsList"
      />
    );
  };
  ...
  const onIsStartPrint = (data) => {
    sendContentForThermalPrinter('', invoiceThermal, data);
  };

  const { setVendorList } = stores.BackToBackPurchaseStore;
  ...
  const closeDialog = () => {
    handleCloseBackToBackPrintSelectionAlertMessage();
  };
  ...
  return (
    <Page className={classes.root} title="Purchases">
      ...
      <Paper className={classes.paperRoot}>
        ...
        {openBackToBackPurchasePrintSelectionAlert === true ? (
          <OpenPreview
            open={true}
            onClose={closeDialog}
            id={printBackToBackPurchaseData.backToBackPurchaseNumber}
            invoiceData={printBackToBackPurchaseData}
            startPrint={false}
          />
        ) : (
          ''
...
export default InjectObserver(BackToBackPurchaseBillList);