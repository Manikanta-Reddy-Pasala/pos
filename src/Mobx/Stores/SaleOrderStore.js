...
class SaleOrderStore {
  ...
  OpenAddSaleOrderInvoice = false;

  OpenSaleOrderBatchList = false;

  OpenSaleOrderSerialList = false;
  ...
  saleOrderTxnEnableFieldsMap = new Map();
  ...
  openSaleOrderLoadingAlertMessage = false;
  openSaleOrderErrorAlertMessage = false;

  openSaleOrderPrintSelectionAlert = false;
  printSaleOrderData = null;
  printSaleOrderBalance = {};
  ...
  setSaleOrderTxnEnableFieldsMap = (saleOrderTransSettingData) => {
    this.saleOrderTransSettingData = saleOrderTransSettingData;
    this.saleOrderTxnEnableFieldsMap = new Map();
    if (saleOrderTransSettingData.businessId.length > 2) {
      const productLevel = saleOrderTransSettingData.enableOnTxn.productLevel;
      productLevel.map((item) => {
        if (this.saleOrderTxnEnableFieldsMap.has(item.name)) {
          this.saleOrderTxnEnableFieldsMap.set(item.name, item.enabled);
        } else {
          this.saleOrderTxnEnableFieldsMap.set(item.name, item.enabled);
        }
      });

      const billLevel = saleOrderTransSettingData.enableOnTxn.billLevel;
      billLevel.map((item) => {
        if (this.saleOrderTxnEnableFieldsMap.has(item.name)) {
          this.saleOrderTxnEnableFieldsMap.set(item.name, item.enabled);
        } else {
      ...
      if (
        !this.isUpdate &&
        this.saleOrderTxnEnableFieldsMap.get('enable_roundoff_default')
      ) {
  ...
  getSaleOrderDetails = async (customerId) => {
  ...
  getSaleOrderList = async (fromDate, toDate) => {
    const db = await Db.get();
  ...
  getSaleOrderListWithLimit = async (fromDate, toDate, pageSize) => {
    const db = await Db.get();
    // console.log('fromDate::', fromDate);
    // console.log('toDate::', toDate);
    var query;
  ...
  handleSaleOrderSearch = async (value) => {
    const db = await Db.get();
    console.log('handleSaleOrderSearch', value);
    let data;
  ...
  handleSaleOrderSearchWithDate = async (value, fromDate, toDate) => {
    const db = await Db.get();
    let data;
    ...
    let query = await db.saleorder.find({
      selector: {
        $and: [
          { businessId: { $eq: businessData.businessId } },
          {
  ...
  handleSaleOrderByEmployeeSearch = async (
    value,
    fromDate,
    toDate,
    employeeId
    ...
    if (employeeId != null) {
      ...
      return this.handleSaleOrderSearchWithDate(value, fromDate, toDate);
    }
  };
  ...
  generateInvoiceNumber = async () => {
    ...
    const id = _uniqueId('so');
    this.saleDetails.sale_order_invoice_number = `${id}${appId}${timestamp}`;
  };
  ...
  viewOrEditSaleOrderTxnItem = async (item) => {
    // console.log('viewOrEditSaleTxnItem::', item);
    await this.viewOrEditItem(item);
  };

  deleteSaleOrderTxnItem = async (item) => {
    ...
    const saleDetails = {
      ...
      sale_order_invoice_number: item.sale_order_invoice_number,
      ...
      convertSOToSale: item.convertSOToSale,
  ...
  saveData = async (isPrint) => {
    if (!this.saleDetails.sale_order_invoice_number) {
    ...
    if (
      this.saleDetails.convertSOToSale === null ||
      this.saleDetails.convertSOToSale === ''
    ) {
      this.saleDetails.convertSOToSale = false;
    }

    if (
      this.saleDetails.saleOrderType === null ||
      this.saleDetails.saleOrderType === ''
    ) {
      this.saleDetails.saleOrderType = 'open';
    }
  ...
  addSaleInformation = async (isPrint) => {
    ...
    if (
      ...
      await this.getSequenceNumber(
        ...
        this.saleDetails.sale_order_invoice_number
      );
    }

    if (this.saleDetails.sequenceNumber === '0') {
      this.saleDetails.sequenceNumber = '';
      this.handleCloseSaleOrderLoadingMessage();
    ...
    if (this.isRestore) {
      await this.markSaleOrderRestored();
    }

    allTxn.saveTxnFromSaleOrder(InsertDoc, db);
    ...
    await audit.addAuditEvent(
      InsertDoc.sale_order_invoice_number,
    ...
    try {
      await db.saleorder.insert(InsertDoc).then((data) => {
        if (isPrint) {
          if (
            ...
            sendContentForThermalPrinter(
              ...
              this.salesInvoiceThermal,
              data,
              this.saleOrderTransSettingData,
              'Sales Order'
            );
          }
        }
        ...
        if (
          ...
          runInAction(() => {
            this.printSaleOrderData = InsertDoc;
            ...
            this.handleOpenSaleOrderPrintSelectionAlertMessage();
          });
        } else {
          this.resetAllData();
          this.closeDialog();
      ...
      await audit.addAuditEvent(
        ...
        userAction,
        JSON.stringify(InsertDoc),
        err.message ? err.message : 'Sale Order Failed',
        InsertDoc.invoice_date
      );
      this.handleCloseSaleOrderLoadingMessage();
      this.handleOpenSaleOrderErrorAlertMessage();
    }
  };
    ...
    if (window.navigator.onLine) {
      ...
      runInAction(() => {
        ...
        this.invoiceData.prefix =
          transSettings.saleOrder.prefixSequence &&
          transSettings.saleOrder.prefixSequence.length > 0
            ? transSettings.saleOrder.prefixSequence[0].prefix
            : '';
      });
      isOnline = true;
    } else {
    ...
    if (paymentInData) {
      const changeData = await (async (oldData) => {
        if (parseFloat(oldData.balance) > 0) {
          ...
          let linkedTxn = {
            linkedId: saleDetails.sale_order_invoice_number,
            date: '',
            linkedAmount: doc.linkedAmount,
            paymentType: 'Sale Order',
            transactionNumber: transactionNumber,
      ...
      const changeData = await ((oldData) => {
        ...
        if (typeof oldData.linkedTxnList === 'undefined') {
          ...
          oldData.linkedTxnList.forEach((element) => {
            if (!(element.linkedId === saleDetails.sale_order_invoice_number)) {
      ...
      .findOne({
        selector: {
          $and: [
            ...
            {
              sales_return_number: { $eq: doc.sale_order_invoice_number }
            },
        ...
        .find({
          selector: {
            $and: [
              ...
              {
                customerId: { $eq: id }
                // customerId: { $eq: 'c131615711116319' }
              },
              { balance: { $gt: 0.0 } }
            ]
          }
          ...
          selector: {
            $and: [
              ...
              },
              {
                linkedTxnList: {
                  $elemMatch: {
                    linkedId: { $eq: saleDetails.sale_order_invoice_number }
                  }
                }
              }
            ]
        ...
        .find({
          selector: {
            $and: [
              ...
              {
                customer_id: { $eq: id }
                // customer_id: { $eq: 'c131615711116319' }
              },
              { balance_amount: { $gt: 0 } }
            ]
          }
  ...
  updateSaleInformation = async (isPrint) => {
    if (
      this.existingSaleData.sale_order_invoice_number &&
      this.existingSaleData.sale_order_invoice_number === 0
    ) {
        ...
        $and: [
          { businessId: { $eq: businessData.businessId } },
          {
            sale_order_invoice_number: {
              $eq: this.existingSaleData.sale_order_invoice_number
            }
          }
        ]
      }
    ...
    query
      ...
      .then(async (data) => {
        ...
        allTxn.deleteAndSaveTxnFromSaleOrder(
          this.existingSaleData,
          txnData,
          db
        );
        ...
        audit.addAuditEvent(
          updateData.sale_order_invoice_number,
          ...
          await query
            ...
            .then(async () => {
              ...
              if (
                ...
                runInAction(() => {
                  this.printSaleOrderData = txnData;
  ...
  deleteData = async (saleDetails, employeeId) => {
    //save to audit
    audit.addAuditEvent(
      saleDetails.sale_order_invoice_number,
    ...
    DeleteDataDoc.transactionId = saleDetails.sale_order_invoice_number;
    DeleteDataDoc.sequenceNumber = saleDetails.sequenceNumber;
    DeleteDataDoc.transactionType = 'Sale Order';
    ...
    DeleteDataDoc.balance = saleDetails.total_amount;
    ...
    allTxn.deleteTxnFromSaleOrder(saleDetails, db);
    ...
    const query = db.saleorder.findOne({
      selector: {
        $and: [
          ...
          {
            sale_order_invoice_number: {
              $eq: saleDetails.sale_order_invoice_number
            }
          }
        ]
      }
        ...
        .then(async (data) => {
          ...
          if (isProductSold === false) {
            ...
            if (existingSaleProduct) {
              ...
              runInAction(() => {
                if (
                  this.saleOrderTxnEnableFieldsMap.get('enable_product_price')
                ) {
                  ...
                  this.saleOrderTxnEnableFieldsMap.get(
  ...
  setInvoiceNumber = (value) => {
    this.saleDetails.sale_order_invoice_number = value;
  };
  ...
  setCustomer = async (customer, isNewCustomer) => {
    ...
    this.saleDetails.customerPincode = customer.pincode;
    this.saleDetails.customerCity = customer.city;
    this.saleDetails.customerAddress = customer.address;
    ...
    customerData.customer_phoneNo = customer.phoneNo;
    customerData.customer_name = customer.name;
    customerData.address = customer.address;
    customerData.pincode = customer.pincode;
    customerData.city = customer.city;
    customerData.state = customer.state;
    customerData.country = customer.country;
    customerData.email = customer.emailId;
    customerData.gstNumber = customer.gstNumber;
    customerData.gstType = customer.gstType;

    this.saleDetails.customer = customerData;
  ...
  selectAddressFromCustomer = (selectedIndex) => {
    if (selectedIndex !== -1 && this.customerAddressType === 'Bill To') {
      this.saleDetails.customerAddress =
        this.customerAddressList[selectedIndex].address;
      this.saleDetails.customerPincode =
        this.customerAddressList[selectedIndex].pincode;
      this.saleDetails.customerCity =
      ...
      if (this.customerAddressList[selectedIndex].state) {
        ...
        if (result) {
          ...
          this.setPlaceOfSupplyState(this.customerAddressList[selectedIndex].state);
        }
      }

      this.saleDetails.customer.address =
        this.customerAddressList[selectedIndex].address;
      this.saleDetails.customer.pincode =
        this.customerAddressList[selectedIndex].pincode;
      this.saleDetails.customer.city =
        this.customerAddressList[selectedIndex].city;
      this.saleDetails.customer.state =
        this.customerAddressList[selectedIndex].state;
      this.saleDetails.customer.country =
        this.customerAddressList[selectedIndex].country;
    }
  ...
  viewOrEditItem = async (item) => {
    ...
    runInAction(() => {
      this.OpenAddSaleOrderInvoice = true;
      this.isUpdate = true;
      this.existingSaleData = item;
      this.paymentLinkTransactions = [];
      this.paymentUnLinkTransactions = [];
  ...
  closeDialog = () => {
    runInAction(() => {
      this.OpenAddSaleOrderInvoice = false;
      this.enabledRow = 0;
    });
  };
  ...
  openForNewSaleOrder = () => {
    ...
    this.isUpdate = false;
    this.OpenAddSaleOrderInvoice = true;
    this.printSaleOrderData = null;
    this.printSaleOrderBalance = {};
    this.openSaleOrderPrintSelectionAlert = false;
  ...
  handleBatchListModalClose = (val) => {
    this.OpenSaleOrderBatchList = false;
    if (val) {
  ...
  selectProductFromSerial = (
    ...
    if (existingSaleProduct) {
      ...
      runInAction(() => {
        if (this.saleOrderTxnEnableFieldsMap.get('enable_product_price')) {
          ...
          this.saleOrderTxnEnableFieldsMap.get('enable_product_price_per_gram')
        ) {
  ...
  handleSerialListModalClose = (val) => {
    runInAction(() => {
      this.OpenSaleOrderSerialList = false;
      this.selectedProduct = {};
    });
  };
      ...
      if (batchData.length > 0) {
        if (batchData.length > 1) {
          runInAction(() => {
            ...
            this.selectedIndex = index;
            this.OpenSaleOrderBatchList = true;
          });
          ...
          runInAction(() => {
            this.converToSelectedProduct(productItem);
            this.selectedIndex = index;
            this.OpenSaleOrderSerialList = true;
          });
      } else {
        runInAction(() => {
          if (this.saleOrderTxnEnableFieldsMap.get('enable_product_price')) {
  ...
  viewAndRestoreSaleOrderItem = async (item) => {
    runInAction(() => {
      this.OpenAddSaleOrderInvoice = true;
      this.isUpdate = false;
      this.isRestore = true;
  ...
  restoreSaleOrderItem = async (item, isRestoreWithNextSequenceNo) => {
    runInAction(() => {
      this.isRestore = true;
      this.isUpdate = false;
      this.existingSaleData = item;
  ...
  markSaleOrderRestored = async () => {
    ...
    const query = db.alltransactionsdeleted.findOne({
      selector: {
        $and: [
          { businessId: { $eq: businessData.businessId } },
          { transactionId: { $eq: this.saleDetails.sale_order_invoice_number } }
        ]
      }
    });
    await query
  ...
  handleOpenSaleOrderLoadingMessage = async () => {
    runInAction(() => {
      this.openSaleOrderLoadingAlertMessage = true;
    });
  };

  handleCloseSaleOrderLoadingMessage = async () => {
    runInAction(() => {
      this.openSaleOrderLoadingAlertMessage = false;
    });
  };

  handleOpenSaleOrderErrorAlertMessage = async () => {
    runInAction(() => {
      this.openSaleOrderErrorAlertMessage = true;
    });
  };

  handleCloseSaleOrderErrorAlertMessage = async () => {
    runInAction(() => {
      this.openSaleOrderErrorAlertMessage = false;
    });
  };

  handleOpenSaleOrderPrintSelectionAlertMessage = async () => {
    runInAction(() => {
      this.openSaleOrderPrintSelectionAlert = true;
    });
  };

  handleCloseSaleOrderPrintSelectionAlertMessage = async () => {
    runInAction(() => {
      this.openSaleOrderPrintSelectionAlert = false;
    });
  };

  resetSaleOrderPrintData = async () => {
    runInAction(() => {
      this.printSaleOrderData = null;
      this.printSaleOrderBalance = {};
      this.openSaleOrderPrintSelectionAlert = false;
  ...
  constructor() {
    makeObservable(this, {
      ...
      isUpdate: observable,
      OpenAddSaleOrderInvoice: observable,
      OpenSaleOrderBatchList: observable,
      OpenSaleOrderSerialList: observable,
      selectedProduct: observable,
      closeDialog: action,
      viewOrEditItem: action,
      openForNewSaleOrder: action,
      selectProduct: action,
      selectProductFromBatch: action,
      setEditTable: action,
      generateInvoiceNumber: action,
      viewOrEditSaleOrderTxnItem: action,
      deleteSaleOrderTxnItem: action,
      FocusLastIndex: observable,
      salesData: observable,
      totalSalesData: observable,
      sales: observable,
      getSaleOrderData: computed,
      getSaleOrderDetails: action,
      addSaleOrderData: action,
      customerList: observable,
      setCustomerList: action,
      getBalanceAfterLinkedAmount: computed,
      dateDropValue: observable,
      getSaleOrderCount: action,
      isSalesList: observable,
      getSaleOrderList: action,
      handleSaleOrderSearch: action,
      ...
      setItemHSN: action,
      saleOrderTxnEnableFieldsMap: observable,
      setSaleOrderTxnEnableFieldsMap: action,
      ...
      viewAndRestoreSaleOrderItem: action,
      restoreSaleOrderItem: action,
      isRestore: observable,
      openSaleOrderPrintSelectionAlert: observable,
      openSaleOrderErrorAlertMessage: observable,
      openSaleOrderLoadingAlertMessage: observable,
...
export default new SaleOrderStore();