...
class DeliveryChallanAddStore {
  saleDetails = {
    ...
    delivery_challan_invoice_number: 0,
    invoice_date: '',
    ...
    discount_type: '',
    sequenceNumber: '',
    convertDCToSale: false,
    convertedFromSale: false,
    status: 'open',
    notes: '',
    customerState: '',
    customerCountry: '',
  ...
  deliveryChallanTxnEnableFieldsMap = new Map();

  taxSettingsData = {};

  OpenDeliveryChallanBatchList = false;
  ...
  OpenAddDeliveryChallanInvoice = false;
  ...
  openDCLoadingAlertMessage = false;
  openDCErrorAlertMessage = false;

  openDCPrintSelectionAlert = false;
  ...
  OpenDeliveryChallanSerialList = false;
  openDeliveryChallanErrorAlertMessage = false;
  errorAlertMessage = '';
  metalList = [];

  getDeliveryChallanCount = async () => {
    const db = await Db.get();
    // console.log('inside sales count');
    const businessData = await Bd.getBusinessData();

    const query = db.deliverychallan.find({
      selector: {
    ...
    await query
      .exec()
      .then((data) => {
        this.isDeliveryChallanList = data.length > 0 ? true : false;
      })
      .catch((err) => {
  ...
  setDeliveryChallanTxnEnableFieldsMap = (deliveryChallanTransSettingData) => {
    this.deliveryChallanTransSettingData = deliveryChallanTransSettingData;

    this.deliveryChallanTxnEnableFieldsMap = new Map();
    if (deliveryChallanTransSettingData.businessId.length > 2) {
      const productLevel =
        deliveryChallanTransSettingData.enableOnTxn.productLevel;
      productLevel.map((item) => {
        if (this.deliveryChallanTxnEnableFieldsMap.has(item.name)) {
          this.deliveryChallanTxnEnableFieldsMap.set(item.name, item.enabled);
        } else {
          this.deliveryChallanTxnEnableFieldsMap.set(item.name, item.enabled);
        }
      });

      const billLevel = deliveryChallanTransSettingData.enableOnTxn.billLevel;
      billLevel.map((item) => {
        if (this.deliveryChallanTxnEnableFieldsMap.has(item.name)) {
          this.deliveryChallanTxnEnableFieldsMap.set(item.name, item.enabled);
        } else {
      ...
      if (
        !this.isUpdate &&
        this.deliveryChallanTxnEnableFieldsMap.get('enable_roundoff_default')
      ) {
  ...
  getDeliveryChallanDetails = async (customerId) => {
    const db = await Db.get();
    const businessData = await Bd.getBusinessData();
  ...
  addDeliveryChallanJSONData = (data) => {
    if (data) {
      runInAction(() => {
        if (data.length > 0) {
  ...
  getDeliveryChallanList = async (fromDate, toDate) => {
    const db = await Db.get();
  ...
  getDeliveryChallanListWithLimit = async (fromDate, toDate, pageSize) => {
    const db = await Db.get();
    // console.log('fromDate::', fromDate);
    // console.log('toDate::', toDate);
    var query;
  ...
  getDeliveryChallanListByCustomer = async (fromDate, toDate, phoneNo) => {
    const db = await Db.get();
    // console.log('fromDate::', fromDate);
    // console.log('toDate::', toDate);
    var query;
    ...
    if (window.navigator.onLine) {
      ...
      runInAction(() => {
        ...
        this.invoiceData.prefix =
          transSettings.deliveryChallan.prefixSequence &&
          transSettings.deliveryChallan.prefixSequence.length > 0
            ? transSettings.deliveryChallan.prefixSequence[0].prefix
            : '';
      });
      isOnline = true;
    } else {
  ...
  handleDeliveryChallanSearch = async (value) => {
    const db = await Db.get();
    let data;
  ...
  generateInvoiceNumber = async () => {
    ...
    this.saleDetails.delivery_challan_invoice_number = `${id}${appId}${timestamp}`;
  };
  ...
  viewOrEditDeliveryChallanTxnItem = async (item) => {
    // console.log('viewOrEditSaleTxnItem::', item);
    await this.viewOrEditItem(item);
  };

  deleteDeliveryChallanTxnItem = async (item) => {
    ...
    const saleDetails = {
      ...
      delivery_challan_invoice_number: item.delivery_challan_invoice_number,
      ...
      status: item.status,
      notes: item.notes,
      customerShippingData: item.customerShippingData,
  ...
  saveData = async (isPrint) => {
    if (!this.saleDetails.delivery_challan_invoice_number) {
    ...
    for (var i = 0; i < this.items.length; i++) {
      ...
      if (item.freeQty === '' || item.freeQty === null) {
        item.freeQty = 0;
      }
    ...
    if (
      this.saleDetails.convertDCToSale === '' ||
      this.saleDetails.convertDCToSale === null
    ) {
      this.saleDetails.convertDCToSale = false;
    }

    if (
      this.saleDetails.convertedFromSale === '' ||
      this.saleDetails.convertedFromSale === null
    ) {
      this.saleDetails.convertedFromSale = false;
    }

    if (this.saleDetails.status === '' || this.saleDetails.status === null) {
    ...
    if (this.items.length > 0) {
      if (this.isUpdate) {
        await this.updateDeliveryChallanInformation(isPrint);
      } else {
        await this.addDeliveryChallanInformation(isPrint);
      }
    }
  };

  addDeliveryChallanInformation = async (isPrint) => {
    const db = await Db.get();
    ...
    if (
      ...
      await this.getSequenceNumber(
        ...
        this.saleDetails.delivery_challan_invoice_number
      );
    }

    if (this.saleDetails.sequenceNumber === '0') {
      this.saleDetails.sequenceNumber = '';
      this.handleCloseDCLoadingMessage();
    ...
    if (
      this.saleToDCDetails.item_list &&
      this.saleToDCDetails.item_list.length > 0
    ) {
      await this.closeSaleTxn();
      InsertDoc.status = 'close';
      InsertDoc.convertDCToSale = true;
      InsertDoc.convertedFromSale = true;
    }

    await allTxn.saveTxnFromDeliveryChallan(InsertDoc, db);

    if (this.isRestore) {
      await this.markDeliveryChallanRestored();
    }
    ...
    await audit.addAuditEvent(
      InsertDoc.delivery_challan_invoice_number,
    ...
    try {
      await db.deliverychallan.insert(InsertDoc).then(async (data) => {
        if (isPrint) {
          if (
            ...
            sendContentForThermalPrinter(
              ...
              this.salesInvoiceThermal,
              data,
              this.deliveryChallanTransSettingData,
              'Delivery Challan'
            );
          }
        }
        ...
        if (
          ...
          runInAction(() => {
            this.printDCData = InsertDoc;
            runInAction(async () => {
              this.isDeliveryChallanList = true;
              this.invoiceData = {};
            });
            ...
            if (this.saveAndNew) {
              ...
              this.openForNewDeliveryChallan();
            }
            this.handleOpenDCPrintSelectionAlertMessage();
          });
        } else {
          runInAction(async () => {
            this.isDeliveryChallanList = true;
      ...
      await audit.addAuditEvent(
        ...
        userAction,
        JSON.stringify(InsertDoc),
        err.message ? err.message : 'Delivery Challan Failed',
        InsertDoc.invoice_date
      );
      this.handleCloseDCLoadingMessage();
      this.handleOpenDCErrorAlertMessage();
    }
  };
  ...
  closeSaleTxn = async () => {
    ...
    const query = db.sales.findOne({
      selector: {
        $and: [
          { businessId: { $eq: businessData.businessId } },
          { invoice_number: { $eq: this.saleToDCDetails.invoice_number } }
        ]
      }
    });
    query
  ...
  updateDeliveryChallanInformation = async (isPrint) => {
    if (
      this.existingSaleData.delivery_challan_invoice_number &&
      this.existingSaleData.delivery_challan_invoice_number === 0
    ) {
        ...
        $and: [
          { businessId: { $eq: businessData.businessId } },
          {
            delivery_challan_invoice_number: {
              $eq: this.existingSaleData.delivery_challan_invoice_number
            }
          }
        ]
      }
    ...
    query
      ...
      .then(async (data) => {
        ...
        allTxn.deleteAndSaveTxnFromDeliveryChallan(
          this.existingSaleData,
          txnData,
          db
        );
        ...
        audit.addAuditEvent(
          updateData.delivery_challan_invoice_number,
  ...
  deleteData = async (saleDetails, employeeId) => {
    //save to audit
    audit.addAuditEvent(
      saleDetails.delivery_challan_invoice_number,
    ...
    DeleteDataDoc.transactionId = saleDetails.delivery_challan_invoice_number;
    DeleteDataDoc.sequenceNumber = saleDetails.sequenceNumber;
    DeleteDataDoc.transactionType = 'Delivery Challan';
    DeleteDataDoc.data = JSON.stringify(saleDetails);
    ...
    const query = db.deliverychallan.findOne({
      selector: {
        $and: [
          ...
          {
            delivery_challan_invoice_number: {
              $eq: saleDetails.delivery_challan_invoice_number
            }
          }
        ]
      }
    ...
    await query
      ...
      .then(async (data) => {
        ...
        await allTxn.deleteTxnFromDeliveryChallan(txnData, db);
  ...
  setInvoiceNumber = (value) => {
    this.saleDetails.delivery_challan_invoice_number = value;
  };
  ...
  setCustomer = async (customer, isNewCustomer) => {
    ...
    this.saleDetails.customer_address = customer.shippingAddress;
    this.saleDetails.customer_pincode = customer.shippingPincode;
    this.saleDetails.customer_city = customer.shippingCity;
    this.saleDetails.customerState = customer.shippingState;
    this.saleDetails.customerCountry = customer.shippingCountry;

    let customerShippingData = {
      customer_id: '',
      customer_phoneNo: '',
      customer_name: '',
      address: '',
    ...
    customerShippingData.customer_id = customer.id;
    customerShippingData.customer_phoneNo = customer.phoneNo;
    customerShippingData.customer_name = customer.name;
    customerShippingData.address = customer.shippingAddress;
    customerShippingData.pincode = customer.shippingPincode;
    customerShippingData.city = customer.shippingCity;
    customerShippingData.state = customer.shippingState;
    customerShippingData.country = customer.shippingCountry;
    customerShippingData.email = customer.emailId;

    this.saleDetails.customerShippingData = customerShippingData;
  ...
  selectAddressFromCustomer = (selectedIndex) => {
    ...
    this.saleDetails.customerShippingData.address =
      this.customerAddressList[selectedIndex].address;
    this.saleDetails.customerShippingData.pincode =
      this.customerAddressList[selectedIndex].pincode;
    this.saleDetails.customerShippingData.city =
      this.customerAddressList[selectedIndex].city;
    this.saleDetails.customerShippingData.state =
      this.customerAddressList[selectedIndex].state;
    this.saleDetails.customerShippingData.country =
      this.customerAddressList[selectedIndex].country;
  ...
  deleteDeliveryChallanItem = async (item) => {
    this.items = item.item_list;
    const saleDetails = {
      customer_id: item.customer_id,
      customer_name: item.customer_name,
  ...
  viewOrEditItem = async (item) => {
    runInAction(() => {
      this.OpenAddDeliveryChallanInvoice = true;
      this.isUpdate = true;
      this.existingSaleData = item;
      this.salesInvoiceRegular = {};
      this.salesInvoiceThermal = {};
  ...
  closeDialog = () => {
    runInAction(() => {
      this.OpenAddDeliveryChallanInvoice = false;
      this.enabledRow = 0;
    });
  };

  openForNewDeliveryChallan = () => {
  ...
  handleBatchListModalClose = (val) => {
    this.OpenDeliveryChallanBatchList = false;
    if (val) {
      ...
      if (batchData.length > 0) {
        if (batchData.length > 1) {
          runInAction(() => {
            ...
            this.selectedIndex = index;
            this.OpenDeliveryChallanBatchList = true;
          });
        } else if (batchData.length === 1) {
          let firstBatchData = batchData[0];
          runInAction(() => {
            if (
              this.deliveryChallanTxnEnableFieldsMap.get('enable_product_price')
            ) {
              ...
              this.deliveryChallanTxnEnableFieldsMap.get(
        ...
        runInAction(() => {
          this.converToSelectedProduct(productItem);
          this.selectedIndex = index;
          this.OpenDeliveryChallanSerialList = true;
        });
      } else {
        runInAction(() => {
          if (
  ...
  createDeliveryChallanFromSale = async (item) => {
    this.saleToDCDetails = item;
    ...
    customerShippingData.customer_id = item.shipToCustomerId;
    customerShippingData.customer_phoneNo = item.shipToCustomerPhoneNo;
    customerShippingData.customer_name = item.shipToCustomerName;
    customerShippingData.address = item.shipToCustomerAddress;
    customerShippingData.pincode = item.shipToCustomerPincode;
    customerShippingData.city = item.shipToCustomerCity;
    customerShippingData.state = item.shipToCustomerState;
    customerShippingData.country = item.shipToCustomerCountry;
    customerShippingData.email = item.shipToCustomerEmailId;

    const saleDetails = {
      customer_id: item.shipToCustomerId,
      customer_name: item.shipToCustomerName,
      customerGSTNo: item.shipToCustomerGSTNo,
      customerGstType: item.shipToCustomerGstType,
      customer_phoneNo: item.shipToCustomerPhoneNo,
      customer_emailId: item.shipToCustomerEmailId,
      customer_address: item.shipToCustomerAddress,
      customer_pincode: item.shipToCustomerPincode,
      customer_city: item.shipToCustomerCity,
      customerState: item.shipToCustomerState,
      customerCountry: item.shipToCustomerCountry,
      is_roundoff: item.is_roundoff,
      round_amount: item.round_amount,
      total_amount: item.total_amount,
    ...
    if (item.posId === 0) {
      for (let i of item.item_list) {
        i.isEdit = false;
        if (i.qty > 0) {
          let item = await this.convertSaleItemToDcItem(i);
    ...
    if (
      ...
      if (item.shipToCustomerGSTNo && item.shipToCustomerGSTNo !== '') {
        let customerExtractedStateCode = item.shipToCustomerGSTNo.slice(0, 2);
        if (
          businessStateCode !== '' &&
      ...
      } else if (item.shipToCustomerState && item.shipToCustomerState !== '') {
        let result = getStateList().find((e) => e.code === businessStateCode);
        if (result) {
          let businessState = result.name;
          if (item.shipToCustomerState === businessState) {
            this.setCGSTSGSTEnabledByPOS(true);
          } else {
            this.setCGSTSGSTEnabledByPOS(false);
          }
  ...
  convertSaleItemToDcItem = async (i) => {
    let item = {};
    ...
    item.mrp = i.mrp;
    item.purchased_price = i.purchased_price;
    item.offer_price = i.offer_price;
    item.mrp_before_tax = i.mrp_before_tax;
    item.qty = i.qty;
    item.cgst = i.cgst;
    item.sgst = i.sgst;
    item.igst = i.igst;
    item.cess = i.cess;
    item.taxType = i.taxType;
    item.igst_amount = i.igst_amount;
    item.cgst_amount = i.cgst_amount;
    item.sgst_amount = i.sgst_amount;
    item.taxIncluded = i.taxIncluded;
    item.discount_percent = i.discount_percent;
    item.discount_amount = i.discount_amount;
    item.discount_amount_per_item = i.discount_amount_per_item;
    item.discount_type = i.discount_type;
    item.amount = i.amount;
    item.roundOff = i.roundOff;
    item.isEdit = i.isEdit;
    item.stockQty = i.stockQty;
    item.vendorName = i.vendorName;
    item.vendorPhoneNumber = i.vendorPhoneNumber;
    item.brandName = i.brandName;
    item.categoryLevel2 = i.categoryLevel2;
    item.categoryLevel2DisplayName = i.categoryLevel2DisplayName;
    item.categoryLevel3 = i.categoryLevel3;
    item.categoryLevel3DisplayName = i.categoryLevel3DisplayName;
    item.wastagePercentage = i.wastagePercentage;
    item.wastageGrams = i.wastageGrams;
    item.grossWeight = i.grossWeight;
    item.netWeight = i.netWeight;
    item.purity = i.purity;
    item.hsn = i.hsn;
    item.makingChargePercent = i.makingChargePercent;
    item.makingChargeAmount = i.makingChargeAmount;
    item.makingChargeType = i.makingChargeType;
    item.makingChargePerGramAmount = i.makingChargePerGramAmount;
    item.serialOrImeiNo = i.serialOrImeiNo;
    item.freeQty = i.freeQty;
    item.unitConversionQty = i.unitConversionQty;
    item.primaryUnit = i.primaryUnit;
    item.secondaryUnit = i.secondaryUnit;
    item.qtyUnit = i.qtyUnit;
    item.units = i.units && i.units.length > 2 ? i.units.slice(0, 2) : i.units;
    item.originalMrpWithoutConversionQty = i.originalMrpWithoutConversionQty;
    item.mfDate = i.mfDate;
    item.expiryDate = i.expiryDate;
    item.rack = i.rack;
    item.warehouseData = i.warehouseData;
    item.batchNumber = i.batchNumber;
    item.modelNo = i.modelNo;
    item.pricePerGram = i.pricePerGram;
    item.stoneWeight = i.stoneWeight;
    item.stoneCharge = i.stoneCharge;
    item.serialOrImeiNo = i.serialOrImeiNo;
    item.itemNumber = i.itemNumber;
    item.originalDiscountPercent = i.originalDiscountPercent;
  ...
  handleDeliveryChallanByEmployeeSearch = async (
    value,
    fromDate,
    toDate,
    employeeId
  ...
  viewAndRestoreDeliveryChallanItem = async (item) => {
    runInAction(() => {
      this.OpenAddDeliveryChallanInvoice = true;
      this.isUpdate = false;
      this.isRestore = true;
  ...
  restoreDeliveryChallanItem = async (item, isRestoreWithNextSequenceNo) => {
    runInAction(() => {
      this.isRestore = true;
      this.isUpdate = false;
      this.existingSaleData = item;
  ...
  markDeliveryChallanRestored = async () => {
    const db = await Db.get();
    const businessData = await Bd.getBusinessData();
    ...
    const query = db.alltransactionsdeleted.findOne({
      selector: {
        $and: [
          ...
          {
            transactionId: {
              $eq: this.saleDetails.delivery_challan_invoice_number
            }
          }
        ]
      }
  ...
  handleOpenDCLoadingMessage = async () => {
    runInAction(() => {
      this.openDCLoadingAlertMessage = true;
    });
  };

  handleCloseDCLoadingMessage = async () => {
    runInAction(() => {
      this.openDCLoadingAlertMessage = false;
    });
  };

  handleOpenDCErrorAlertMessage = async () => {
    runInAction(() => {
      this.openDCErrorAlertMessage = true;
    });
  };

  handleCloseDCErrorAlertMessage = async () => {
    runInAction(() => {
      this.openDCErrorAlertMessage = false;
    });
  };

  handleOpenDCPrintSelectionAlertMessage = async () => {
    runInAction(() => {
      this.openDCPrintSelectionAlert = true;
    });
  };

  handleCloseDCPrintSelectionAlertMessage = async () => {
    runInAction(() => {
      this.openDCPrintSelectionAlert = false;
    });
  };

  resetDCPrintData = async () => {
    runInAction(() => {
      this.printDCData = null;
      this.printDCBalanceData = {};
      this.openDCPrintSelectionAlert = false;
    });
  };
          ...
          if (isProductSold === false) {
            ...
            if (existingSaleProduct) {
              runInAction(() => {
                ...
                this.openDeliveryChallanErrorAlertMessage = true;
                this.items[index].serialOrImeiNo = '';
              });
            } else {
              this.converToSelectedProduct(actualProduct);
  ...
  handleSerialListModalClose = (val) => {
    runInAction(() => {
      this.OpenDeliveryChallanSerialList = false;
      this.selectedProduct = {};
    });
  };
  ...
  handleOpenDeliveryChallanErrorAlertMessage = async (message) => {
    runInAction(() => {
      this.openDeliveryChallanErrorAlertMessage = true;
  ...
  handleCloseDeliveryChallanErrorAlertMessage = async () => {
    runInAction(() => {
      this.openDeliveryChallanErrorAlertMessage = false;
      this.errorAlertMessage = '';
    });
  };
  ...
  constructor() {
    makeObservable(this, {
      ...
      isUpdate: observable,
      OpenAddDeliveryChallanInvoice: observable,
      OpenDeliveryChallanBatchList: observable,
      selectedProduct: observable,
      closeDialog: action,
      viewOrEditItem: action,
      openForNewDeliveryChallan: action,
      selectProduct: action,
      selectProductFromBatch: action,
      setEditTable: action,
      generateInvoiceNumber: action,
      viewOrEditDeliveryChallanTxnItem: action,
      deleteDeliveryChallanTxnItem: action,
      FocusLastIndex: observable,
      salesData: observable,
      totalSalesData: observable,
      sales: observable,
      getDeliveryChallanData: computed,
      getDeliveryChallanDetails: action,
      addDeliveryChallanData: action,
      customerList: observable,
      setCustomerList: action,
      dateDropValue: observable,
      getDeliveryChallanCount: action,
      isDeliveryChallanList: observable,
      getDeliveryChallanList: action,
      handleDeliveryChallanSearch: action,
      ...
      setItemHSN: action,
      deliveryChallanTxnEnableFieldsMap: observable,
      setDeliveryChallanTxnEnableFieldsMap: action,
      taxSettingsData: observable,
      setTaxSettingsData: action,
      setMakingChargeAmount: action,
      setMakingCharge: action,
      createDeliveryChallanFromSale: action,
      setMakingChargePerGramAmount: action,
      viewAndRestoreDeliveryChallanItem: action,
      restoreDeliveryChallanItem: action,
      isRestore: observable,
      openDCPrintSelectionAlert: observable,
      printDCData: observable,
      openDCErrorAlertMessage: observable,
      openDCLoadingAlertMessage: observable,
      ...
      OpenDeliveryChallanSerialList: observable,
      openDeliveryChallanErrorAlertMessage: observable,
      errorAlertMessage: observable
    });
  }
}

export default new DeliveryChallanAddStore();