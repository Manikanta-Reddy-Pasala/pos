...
import * as deleteTxn from 'src/components/Helpers/DeleteTxnHelper';
...
import ComplaintBooking from './classes/ComplaintBooking';
import ComplaintBookingItem from './classes/ComplaintBookingItem';
import {
  saveComplaintBooking,
  updateComplaintBooking,
  deleteComplaintBookingById
} from 'src/components/Helpers/dbQueries/complaintbooking';

class ComplaintBookingStore {
  newVendor = false;

  isUpdate = false;

  openAddComplaintBooking = false;
  ...
  complaintBookingData = null;
  dateDropValue = null;
  vendorList = [];
  isComplaintBookingList = false;

  complaintBookingInvoiceRegular = {};
  complaintBookingInvoiceThermal = {};

  taxSettingsData = {};

  addNewRowEnabled = false;

  printComplaintBookingData = null;

  openComplaintBookingLoadingAlertMessage = false;
  openComplaintBookingErrorAlertMessage = false;

  openComplaintBookingPrintSelectionAlert = false;
  ...
  addPurchasesData = (data) => {
    if (data) {
      runInAction(() => {
        if (data.length > 0) {
          this.complaintBookingData = data.map((item) => item.toJSON());
        } else {
          this.complaintBookingData = [];
        }
      });
    }
  };
  ...
  get getPurchasesData() {
    return this.complaintBookingData ? this.complaintBookingData : [];
  }
  ...
  getComplaintBookingsCount = async () => {
    const db = await Db.get();
    const businessData = await Bd.getBusinessData();

    await db.complaintbookings
      .find({
        selector: {
      ...
      .exec()
      .then((data) => {
        this.isComplaintBookingList = data.length > 0 ? true : false;
      })
      .catch((err) => {
  ...
  addNewItem = (status, focusIndexStatus, isBarcode) => {
    ...
    if (lastItem) {
      if (lastItem.qty > 0) {
        this.items.push(new ComplaintBookingItem().getgetDefaultValues());
  ...
  openForNewComplaintBooking = () => {
    runInAction(() => {
      this.isUpdate = false;
      this.openAddComplaintBooking = true;

      this.complaintBookingInvoiceRegular = {};
      this.complaintBookingInvoiceThermal = {};

      this.billDetails = new ComplaintBooking().getgetDefaultValues();

      this.items = [new ComplaintBookingItem().getgetDefaultValues()];
    });
  };
  ...
  resetSingleProduct = (index) => {
    let defaultItem = new ComplaintBookingItem().getgetDefaultValues();
  ...
  setInvoiceRegularSetting = (invoiceRegular) => {
    runInAction(() => {
      this.complaintBookingInvoiceRegular = invoiceRegular;
    });
  };

  setInvoiceThermalSetting = (invoicThermal) => {
    runInAction(() => {
      this.complaintBookingInvoiceThermal = invoicThermal;
    });
  };
  ...
  generateBillNumber = async () => {
    ...
    runInAction(() => {
      this.billDetails.bookingId = `${id}${appId}${timestamp}`;
    });
  };
  ...
  closeDialogForSaveAndPrint = () => {
    this.handleCloseComplaintBookingLoadingMessage();
    if (this.isUpdate) {
      this.isUpdate = false;
      this.closeDialog();
      if (this.saveAndNew) {
        this.saveAndNew = false;
        this.openForNewComplaintBooking();
      }

      runInAction(async () => {
        this.isComplaintBookingList = true;
      });
  ...
  resetComplaintBookingPrintData = async () => {
    runInAction(() => {
      this.printComplaintBookingData = {};
    });
  };

  handleOpenComplaintBookingLoadingMessage = async () => {
    runInAction(() => {
      this.openComplaintBookingLoadingAlertMessage = true;
    });
  };

  handleCloseComplaintBookingLoadingMessage = async () => {
    runInAction(() => {
      this.openComplaintBookingLoadingAlertMessage = false;
    });
  };

  handleOpenComplaintBookingErrorAlertMessage = async () => {
    runInAction(() => {
      this.openComplaintBookingErrorAlertMessage = true;
    });
  };

  handleCloseComplaintBookingErrorAlertMessage = async () => {
    runInAction(() => {
      this.openComplaintBookingErrorAlertMessage = false;
    });
  };

  handleOpenComplaintBookingPrintSelectionAlertMessage = async () => {
    runInAction(() => {
      this.openComplaintBookingPrintSelectionAlert = true;
    });
  };

  handleCloseComplaintBookingPrintSelectionAlertMessage = async () => {
    runInAction(() => {
      this.openComplaintBookingPrintSelectionAlert = false;
    });
  };

  closeDialog = () => {
    runInAction(() => {
      this.openAddComplaintBooking = false;
      this.enabledRow = 0;
    });
  };
  ...
  get getTotalAmount() {
    ...
    runInAction(() => {
      this.billDetails.totalAmount = totalAmount;
    });

    return parseFloat(this.billDetails.totalAmount).toFixed(2);
  }
  ...
  saveData = async (isPrint) => {
    ...
    if (
      this.billDetails.totalAmount === null ||
    ...
    if (this.isUpdate) {
      ...
      await this.saveComplaintBooking(isPrint);
    }
  };

  saveComplaintBooking = async (isPrint) => {
    const db = await Db.get();
    ...
    if (
      this.billDetails.bookingId === '' ||
      this.billDetails.bookingId === undefined
    ) {
    ...
    const status = await saveComplaintBooking(InsertDoc);
    if (status) {
      if (
        isPrint &&
        this.complaintBookingInvoiceThermal &&
        this.complaintBookingInvoiceThermal.boolDefault
      ) {
        ...
        if (
          this.complaintBookingInvoiceRegular &&
          this.complaintBookingInvoiceRegular.boolDefault &&
          isPrint
        ) {
          this.printComplaintBookingData = InsertDoc;
          this.closeDialogForSaveAndPrint();
          this.handleOpenComplaintBookingPrintSelectionAlertMessage();
        } else {
          this.closeDialog();
          this.handleCloseComplaintBookingLoadingMessage();
      ...
      this.handleCloseComplaintBookingLoadingMessage();
      this.handleOpenComplaintBookingErrorAlertMessage();
    }
  };
  ...
  updatePurchase = async (isPrint) => {
    ...
    const query = db.complaintbookings.findOne({
      selector: {
        $and: [
          { businessId: { $eq: businessData.businessId } },
          {
            bookingId: {
              $eq: this.existingBillData.bookingId
            }
          }
        ]
      }
        ...
        await query
          ...
          .then(async () => {
            ...
            if (
              ...
              this.printComplaintBookingData = auditData;
              this.closeDialogForSaveAndPrint();
              this.handleOpenComplaintBookingPrintSelectionAlertMessage();
            } else {
              this.isUpdate = false;
  ...
  deleteComplaintBookingTxnItem = async (item) => {
    await this.deletePurchaseEntry(item);
  };

  deletePurchaseEntry = async (item) => {
    const tempBillDetails = new ComplaintBooking().convertTypes(
    ...
    if (!this.billDetails.bookingId) {
      console.log('bookingId not present');
      return;
    }
    ...
    let txnData = {};
    txnData = this.billDetails;
    txnData.itemList = this.items;
    txnData.employeeId = item.employeeId;

    DeleteDataDoc.transactionId = this.billDetails.bookingId;
    DeleteDataDoc.sequenceNumber = '';
    DeleteDataDoc.transactionType = 'Complaint Booking';
    DeleteDataDoc.data = JSON.stringify(txnData);
    DeleteDataDoc.total = this.billDetails.totalAmount;
    DeleteDataDoc.createdDate = this.billDetails.date;
    ...
    await deleteComplaintBookingById(this.billDetails.bookingId, this.billDetails);
  ...
  viewOrEditComplaintBookingTxnItem = async (item) => {
  ...
  viewOrEditItem = async (item) => {
    ...
    this.billDetails = new ComplaintBooking().convertTypes(
      JSON.parse(JSON.stringify(item))
    );
  };
  ...
  generateProductId = async () => {
    ...
    const businessData = await Bd.getBusinessData();
    const appId = businessData.posDeviceId;
    const id = _uniqueId('cb');
    return `${id}${appId}${timestamp}`;
  };

  constructor() {
    this.billDetails = new ComplaintBooking().getDefaultValues();
    this.items = [new ComplaintBookingItem().getDefaultValues()];

    makeObservable(this, {
      billDetails: observable,
      items: observable,
      isUpdate: observable,
      openAddComplaintBooking: observable,
      vendorList: observable,
      getVendorList: computed,
      dateDropValue: observable,
      getDateDropValue: computed,
      complaintBookingData: observable,
      isComplaintBookingList: observable,
      enabledRow: observable,
      OpenBatchList: observable,
      selectedProduct: observable,
      FocusLastIndex: observable,
      taxSettingsData: observable,
      openComplaintBookingLoadingAlertMessage: observable,
      openComplaintBookingErrorAlertMessage: observable,
      openComplaintBookingPrintSelectionAlert: observable,
...
export default new ComplaintBookingStore();