...
const LinkPaymentPurchases = ({ open, onClose, isEditAllowed }) => {
  const store = useStore();

  const { openLinkpaymentPage, paymentLinkTransactions, billDetails } = toJS(
  ...
  const handleClick = (event, row) => {
    if (billDetails.balance_amoun > 0) {
    }
  ...
  const autoLinkClicked = () => {
    const totalAmount = billDetails.total_amount;
    const recievedAmount = parseFloat(billDetails.received_amount) || 0;
    let linkedAmount = parseFloat(billDetails.linked_amount) || 0;
      ...
      <DialogContent>
        <Grid container spacing={3}>
          ...
          <Grid item xs={2} style={{ flexDirection: 'row' }}>
            <Typography>Paid on Purchase</Typography>
            <Typography style={{ marginTop: 5 }}>
          ...
          <Grid item xs={2} style={{ flexDirection: 'row' }}>
            ...
            <Input
              ...
              value={billDetails.linked_amount}
              disabled={true}
              fullWidth
              onChange={(e) => {
...
export default InjectObsrvable(LinkPaymentPurchases);
