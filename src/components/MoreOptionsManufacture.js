...
function Moreoptions(props) {
  ...
  const { deleteManufactureData, updateProductFromManufacture } = store.ProductStore;
  ...
  const confirmDeleteItem = (confirm) => {
    if (confirm) {
      deleteManufactureData(props.item);
  ...
  return (
    <div>
      ...
      <Menu
        ...
        onClose={handleClose}
      >
        <MenuItem key={1} onClick={() => updateProductFromManufacture(props.item)}>
          View/Edit{' '}
        </MenuItem>
