...
function Moreoptions(props) {
  ...
  const { viewAndRestoreCancelledSaleItem } = store.SalesAddStore;
  ...
  const { viewAndRestoreCancelledSaleReturnItem } = store.ReturnsAddStore;
  ...
  const viewAndRestore = async () => {
    ...
    switch (props.item.transactionType) {
      case 'Sales':
        try {
          viewAndRestoreCancelledSaleItem(JSON.parse(props.item.data));
        } catch (e) {
      ...
      case 'Sales Return':
        try {
          viewAndRestoreCancelledSaleReturnItem(JSON.parse(props.item.data));
        } catch (e) {
          console.error(' Error: ', e.message);
        }
        break;
