...
function MoreOptionsDeliveryChallan(props) {
  ...
  const { viewOrEditItem, deleteDeliveryChallanItem, duplicate } =
    store.DeliveryChallanStore;
  const { convertDeliveryChallanToSale } = store.SalesAddStore;
  ...
  const downloadPDF = async () => {
    ...
    await axios
      .get(`${API_SERVER}/pos/v1/download/deliveryChallan`, {
        ...
        params: {
          ...
          invoiceId: props.item.delivery_challan_invoice_number
        }
      })
      .then((res) => {
        ...
        let fileName =
          'Delivery_Challan_' +
          props.item.delivery_challan_invoice_number +
          '.pdf';
      ...
      <div>
        ...
        >
          ...
          {props.item.status === 'open' && (
            <MenuItem
              key={4}
              onClick={() => convertDeliveryChallanToSale(props.item)}
            >
              {' '}
              Convert To Sale
            </MenuItem>
        ...
        {openDialogName === 'preview' ? (
          <OpenPreview
            ...
            txnSettings={deliveryChallanTransSettingData}
...
export default injectWithObserver(MoreOptionsDeliveryChallan);