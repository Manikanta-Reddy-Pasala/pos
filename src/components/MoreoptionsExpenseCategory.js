...
function Moreoptions(props) {
  ...
  const { viewExpenseCategory, deleteExpenseCategory } = store.ExpensesStore;
  const { expenseList } = toJS(store.ExpensesStore);
  ...
  const confirmDeleteItem = (confirm) => {
    if (confirm) {
      deleteExpenseCategory(props.item);
    ...
    <div>
      ...
      >
        <MenuItem key={1} onClick={() => {
          viewExpenseCategory(props.item)
          setAnchorEl(null);
        }
          }>
          View/Edit{' '}
