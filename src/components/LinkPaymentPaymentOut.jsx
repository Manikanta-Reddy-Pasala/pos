...
const LinkPaymentPaymentOut = ({ open, onClose, isEditAllowed }) => {
  const store = useStore();

  const { openLinkpaymentPage, paymentLinkTransactions, paymentDetails } = toJS(
  ...
  const handleClick = (event, row) => {
    if (paymentDetails.balance_amoun > 0) {
    }
  ...
  const autoLinkClicked = () => {
    const totalAmount = paymentDetails.total_amount;
    const recievedAmount = parseFloat(paymentDetails.received_amount) || 0;
    let linkedAmount = parseFloat(paymentDetails.linked_amount) || 0;
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={2} style={{ flexDirection: 'row' }}>
            ...
            <Input
              ...
              value={paymentDetails.linked_amount}
              fullWidth
              disabled={true}
              onChange={(e) => {
...
export default InjectObsrvable(LinkPaymentPaymentOut);
