...
function Moreoptions(props) {
  ...
  const { viewOrEditAdjustmentItem, deleteAdjustmentItem } = store.ProductStore;
  ...
  const confirmDeleteItem = (confirm) => {
    if (confirm) {
      deleteAdjustmentItem(props.item);
  ...
  return (
    <div>
      ...
      <Menu
        ...
        onClose={handleClose}
      >
        <MenuItem key={1} onClick={() => viewOrEditAdjustmentItem(props.item)}>
          View/Edit{' '}
        </MenuItem>
