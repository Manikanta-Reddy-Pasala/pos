...
function Moreoptions(props) {
  ...
  const { viewOrEditItem, deletePaymentIn } = store.PaymentInStore;
  ...
  const { recievePaymentOpen } = store.PaymentInStore;
  const { getPaymentInPaymentHistory, resetPaymentHistory } =
  ...
  const openRecievePayment = (item) => {
    recievePaymentOpen(item);
    setOpenDialog('recievePayment');
    handleClose();
  };
  ...
  const downloadPDF = async () => {
    ...
    await axios
      .get(`${API_SERVER}/pos/v1/download/paymentIn`, {
        ...
        params: {
          ...
          receiptNumber: props.item.receiptNumber,
      ...
      .then((res) => {
        ...
        link.href = url;
        let fileName = 'Payment_In_' + props.item.receiptNumber + '.pdf';
    ...
    <div>
      <div>
        ...
        {openDialogName === 'paymenthistory' ? (
          <PaymentHistory
            open={openDialogName === 'paymenthistory'}
            onClose={closeDialog}
            index={props.index}
          />
        ) : null}

        {openDialogName === 'recievePayment' ? (
          <RecievePayment
            open={openDialogName === 'recievePayment'}
            onClose={closeDialog}
            index={props.index}
          />
        ) : null}
