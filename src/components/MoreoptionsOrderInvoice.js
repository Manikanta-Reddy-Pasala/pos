...
function Moreoptions(props) {
  ...
  const { viewOrEditItem, deleteOrderInvoice } = store.JobWorkStore;

  const { convertToReceipt } = store.JobWorkReceiptStore;
  ...
  const confirmDeleteItem = async (confirm) => {
    if (confirm) {
      await deleteOrderInvoice(props.item);
  ...
  const downloadPDF = async () => {
    ...
    await axios
      .get(`${API_SERVER}/pos/v1/download/jobWorkOut`, {
      ...
      .then((res) => {
        ...
        link.href = url;
        let fileName = 'Job_Work_Out_' + props.item.id + '.pdf';
    ...
    <div>
      <div>
        ...
        >
          ...
          {!(props.item.fullReceipt || props.item.partialReceipt) &&
          ...
          {!props.item.fullReceipt && (
            <MenuItem key={3} onClick={() => convertToReceipt(props.item)}>
              Convert to Receipt{' '}
            </MenuItem>
          )}
