...
const SaleQuotationSerialListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectProductFromSerial } = stores.SalesQuotationAddStore;

  const { handleSerialListModalClose, OpenSaleQuotationSerialList } = stores.SalesQuotationAddStore;
  ...
  return (
    <Dialog
      open={OpenSaleQuotationSerialList}
      fullWidth={true}
      maxWidth={'sm'}
      onKeyDown={(e) => {
...
export default injectWithObserver(SaleQuotationSerialListModal);