...
const PurchaseOrderBillSerialListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const inputRef = useRef([]);

  const { OpenPurchaseOrderSerialList } = toJS(stores.PurchaseOrderStore);
  ...
  return (
    <Dialog open={OpenPurchaseOrderSerialList} fullWidth={true} maxWidth={'sm'}>
...
export default injectWithObserver(PurchaseOrderBillSerialListModal);