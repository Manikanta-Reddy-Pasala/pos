...
const SaleOrderBatchListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectProductFromBatch } = stores.SaleOrderStore;

  const { handleBatchListModalClose, OpenSaleOrderBatchList } =
...
export default injectWithObserver(SaleOrderBatchListModal);