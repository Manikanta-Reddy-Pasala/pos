...
const QuotationBatchListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectProductFromBatch } = stores.SalesQuotationAddStore;

  const { handleBatchListModalClose, OpenQuotationBatchList } =
...
export default injectWithObserver(QuotationBatchListModal);