...
const PurchaseReturnSerialListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const inputRef = useRef([]);

  const { OpenPurchaseReturnSerialList } = toJS(stores.PurchasesReturnsAddStore);

  const {
    handleSerialListModalClose,
    selectProductFromSerial,
    modifySelectedSerialNoForPurchaseReturn
  } = stores.PurchasesReturnsAddStore;

  return (
    <Dialog open={OpenPurchaseReturnSerialList} fullWidth={true} maxWidth={'sm'}>
...
export default injectWithObserver(PurchaseReturnSerialListModal);