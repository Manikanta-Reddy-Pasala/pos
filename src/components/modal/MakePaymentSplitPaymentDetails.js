...
const MakePaymentSplitPaymentDetails = (props) => {
  ...
  const saveSplitPaymentDetails = async () => {
    if (getTotalAmount() !== parseFloat(paymentDetails.paid)) {
      setErrorAlertProductMessage(
        'Please check the total amount provided. Its not matching the payment out total amount.'
      );
  ...
  const getTotalAmountGiven = () => {
    ...
    return paymentDetails.paid - result;
  };
  ...
  return (
    ...
    >
      ...
      <DialogActions>
        <div>
          <b style={{ marginRight: '20px' }}>TOTAL: {paymentDetails.paid}</b>
...
export default MakePaymentSplitPaymentDetails;