...
const SaleOrderSerialListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectProductFromSerial } = stores.SaleOrderStore;

  const { handleSerialListModalClose, OpenSaleOrderSerialList } = stores.SaleOrderStore;

  useEffect(() => {
    console.log("aaaaa",OpenSaleOrderSerialList);
    setActive(0);
  }, [props]);
...
export default injectWithObserver(SaleOrderSerialListModal);