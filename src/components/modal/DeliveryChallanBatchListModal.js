...
const DeliveryChallanBatchListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectProductFromBatch } = stores.DeliveryChallanStore;

  const { handleBatchListModalClose, OpenDeliveryChallanBatchList } =
  ...
  return (
    <Dialog
      open={OpenDeliveryChallanBatchList}
      fullWidth={true}
      maxWidth={'md'}
      onKeyDown={(e) => {
...
export default injectWithObserver(DeliveryChallanBatchListModal);