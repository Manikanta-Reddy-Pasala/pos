...
const JobWorkInBatchListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectProductFromBatch } = stores.JobWorkInStore;

  const { handleBatchListModalClose, OpenJobWorkInBatchList } =
...
export default injectWithObserver(JobWorkInBatchListModal);