...
const DeliveryChallanSerialListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectProductFromSerial } = stores.DeliveryChallanStore;

  const { handleSerialListModalClose, OpenDeliveryChallanSerialList } = stores.DeliveryChallanStore;
  ...
  return (
    <Dialog
      open={OpenDeliveryChallanSerialList}
      fullWidth={true}
      maxWidth={'sm'}
      onKeyDown={(e) => {
...
export default injectWithObserver(DeliveryChallanSerialListModal);