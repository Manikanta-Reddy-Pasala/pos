...
const SerialListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectProductFromSerial } = stores.SalesAddStore;

  const { handleSerialListModalClose, OpenSerialList } = stores.SalesAddStore;

  useEffect(() => {
    console.log('aaaaa', OpenSerialList);
    setActive(0);
  }, [props]);
...
export default injectWithObserver(SerialListModal);