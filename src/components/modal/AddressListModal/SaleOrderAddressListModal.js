...
const SaleOrderAddressListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectAddressFromCustomer, handleCloseAddressList} = stores.SaleOrderStore;

  const { openAddressList } = toJS(stores.SaleOrderStore);
...
export default injectWithObserver(SaleOrderAddressListModal);
