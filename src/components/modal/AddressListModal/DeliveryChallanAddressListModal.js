...
const DeliveryChallanAddressListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectAddressFromCustomer, handleCloseAddressList} = stores.DeliveryChallanStore;
  const { openAddressList } = toJS(stores.DeliveryChallanStore);
          ...
          {props.addressList
            ? props.addressList.map((option, index) => (
                <li
                  ...
                  onClick={function (event) {
                    setActive(index);
                    selectAddressFromCustomer(index);
                  }}
...
export default injectWithObserver(DeliveryChallanAddressListModal);
