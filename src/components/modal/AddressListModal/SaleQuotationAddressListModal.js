...
const SaleQuotationAddressListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectAddressFromCustomer, handleCloseAddressList} = stores.SalesQuotationAddStore;
  const { openAddressList } = toJS(stores.SalesQuotationAddStore);
...
export default injectWithObserver(SaleQuotationAddressListModal);
