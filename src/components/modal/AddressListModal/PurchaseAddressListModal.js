...
const PurchaseAddressListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectAddressFromVendor, handleCloseAddressList} = stores.PurchasesAddStore;

  const { openAddressList } = toJS(stores.PurchasesAddStore);
...
export default injectWithObserver(PurchaseAddressListModal);
