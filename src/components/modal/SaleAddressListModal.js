...
const SaleAddressListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectAddressFromCustomer, handleCloseAddressList} = stores.SalesAddStore;

  const { openAddressList } = toJS(stores.SalesAddStore);
...
export default injectWithObserver(SaleAddressListModal);
