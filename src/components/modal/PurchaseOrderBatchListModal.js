...
const PurchaseOrderBatchListModal = (props) => {
  const classes = useStyles();
  const stores = useStore();
  const [active, setActive] = useState(0);
  const inputRef = useRef([]);

  const { selectProductFromBatch } = stores.PurchaseOrderStore;

  const { handleBatchListModalClose, OpenPurchaseOrderBatchList } =
...
export default injectWithObserver(PurchaseOrderBatchListModal);