...
function Moreoptions(props) {
  ...
  const { updateProduct, handleOpenSerialModel } = store.ProductStore;
  ...
  const confirmDeleteItem = (confirm) => {
    if (confirm) {
      let itemToDeleteSerialData = props.item;
      itemToDeleteSerialData.serialData = [];
      updateProduct(itemToDeleteSerialData);
  ...
  return (
    <div>
      ...
      >
        <MenuItem
          key={1}
          onClick={() => handleOpenSerialModel(true, props.item)}
        >
          View/Edit{' '}
        </MenuItem>
