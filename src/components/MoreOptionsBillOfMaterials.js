...
function Moreoptions(props) {
  ...
  const { editRawMAterialModal } = store.RawMaterialsStore;
  ...
  const confirmDeleteItem = (confirm) => {
    if (confirm) {
      ...
      let itemToDeleteRawMaterials = props.item;
      itemToDeleteRawMaterials.rawMaterialData = rMData;
      updateProduct(itemToDeleteRawMaterials);
  ...
  return (
    <div>
      ...
      <Menu
        ...
        onClose={handleClose}
      >
        <MenuItem key={1} onClick={() => editRawMAterialModal(props.item)}>
          View/Edit{' '}
        </MenuItem>
