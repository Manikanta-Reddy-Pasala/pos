...
export const WaitersSchema = {
  title: 'waiters Schema',
  version: 1,
  type: 'object',
  properties: {
    businessId: {
...
const pullQueryBuilderInBackground = greenlet(async (doc, businessData, localStoragePosId) => {
  ...
  const BATCH_SIZE = 30;
  const query = `{
    getWaiters(lastId: "${doc.phoneNumber}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
      businessId
      businessCity
      phoneNumber
      name
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setWaiter($input: WaiterInput) {
...
export const waitersSyncQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.waiters.syncGraphQL({
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validateWaitersDocumentBeforeSync(doc);
      }
    },
    pull: {
