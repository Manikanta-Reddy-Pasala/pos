...
export const ProductTransactionSettingsSchema = {
  title: 'Product Transaction Settings Schema table',
  description: '',
  version: 2,
  type: 'object',
  properties: {
...
const pullQueryBuilderInBackground = greenlet(async (doc, localStoragePosId, businessData) => {
  ...
  const query = `{
      getProductTransactionSettings(lastId: "${doc.businessId}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
        businessId
        businessCity
        updatedAt
        posId
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setProductTransactionSettings($input: ProductTransactionSettingsInput) {
...
export const productTransactionSettingsQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.producttransactionsettings.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validateProductTransactionSettingsDocumentBeforeSync(
          doc
        );
      }
    },
