...
export const SequenceNumbersSchema = {
  title: '',
  description: '',
  version: 2,
  type: 'object',
...
const pullQueryBuilderInBackground = greenlet(async (doc, localStoragePosId, businessData) => {
  ...
  const query = `{
    getSequence( lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
      id
      businessId
      businessCity
      type
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setSequence($input: SequenceInput) {
...
export const sequenceSyncQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.sequencenumbers.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validateSequenceNumbersDocumentBeforeSync(doc);
      }
    },
    pull: {
