...
export const CashAdjustmentsSchema = {
  title: 'cash adjustments table',
  description: 'List of cash adjustments txn',
  version: 3,
  type: 'object',
  properties: {
    businessId: {
  ...
  indexes: ['updatedAt'],
  required: ['businessId', 'businessCity', 'id', 'date', 'updatedAt']
};
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setCashAdjustments($input: CashAdjustmentsInput) {
...
const pullQueryBuilderInBackground = greenlet(async (doc, businessData, localStoragePosId) => {
  ...
  const BATCH_SIZE = 30;
  const query = `{
    getCashAdjustments(lastId: "${doc.id}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
      id
      transactionType
      cashType
      amount
...
export const cashAdjustmentsSyncQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.cashadjustments.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validateCashAdjustmentsDocumentBeforeSync(doc);
      }
    },
    pull: {
