...
export const ExpenseTransactionSettingsSchema = {
  title: 'Expense Transaction Settings Schema table',
  description: '',
  version: 3,
  type: 'object',
  properties: {
...
const pullQueryBuilderInBackground = greenlet(async (doc, businessData, localStoragePosId) => {
  ...
  const BATCH_SIZE = 30;
  const query = `{
      saleExpenseTransactionSettings(lastId: "${doc.businessId}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
        businessId
        businessCity
        updatedAt
        posId
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setExpenseTransactionSettings($input: ExpenseTransactionSettingsInput) {
...
export const expenseTransactionSettingsQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.expensetransactionsettings.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validateExpenseTransactionSettingsDocumentBeforeSync(
          doc
        );
      }
    },
