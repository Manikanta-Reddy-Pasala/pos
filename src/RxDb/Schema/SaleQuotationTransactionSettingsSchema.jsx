...
export const SaleQuotationTransactionSettingsSchema = {
  title: 'Sale Quotation Transaction Settings Schema table',
  description: '',
  version: 2,
  type: 'object',
  properties: {
...
const pullQueryBuilderInBackground = greenlet(async (doc) => {
  ...
  const query = `{
      saleQuotationTransactionSettings(lastId: "${doc.businessId}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
        businessId
        businessCity
        updatedAt
        posId
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setSaleQuotationTransactionSettings($input: SaleQuotationTransactionSettingsInput) {
...
export const saleQuotationTransactionSettingsQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.salequotationtransactionsettings.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validateSaleQuotationTransactionSettingsDocumentBeforeSync(
          doc
        );
      }
    },
