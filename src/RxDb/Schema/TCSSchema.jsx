...
export const TCSSchema = {
  title: 'tcs table',
  description: 'List of tcs txn',
  version: 3,
  type: 'object',
  properties: {
    businessId: {
...
const pullQueryBuilderInBackground = greenlet(async (doc, businessData, localStoragePosId) => {
  ...
  const BATCH_SIZE = 30;
  const query = `{
    getTCS(lastId: "${doc.id}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
      id
      code
      name
      rate
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setTCS($input: TCSInput) {
      setTCS(tcs: $input) {
        id,
        updatedAt
      }
...
export const tcsSyncQueryBuilder = (db, syncURL, batchSize, posId, token) => {
  return db.tcs.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validateTCSDocumentBeforeSync(doc);
      }
    },
    pull: {
