...
export const EmployeeTypesSchema = {
  title: 'Employee Type table',
  description: 'List of Employee Type',
  version: 0,
  type: 'object',
  properties: {
    businessId: {
...
const pullQueryBuilderInBackground = greenlet(async (doc, businessData, localStoragePosId) => {
  ...
  const BATCH_SIZE = 30;
  const query = `{
    getEmployeeTypes(lastId: "${doc.id}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
      id
      name
      updatedAt
      businessId
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setEmployeeTypes($input: EmployeeTypesInput) {
...
export const employeeTypesSyncQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.employeetypes.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validateEmployeeTypesDocumentBeforeSync(doc);
      }
    },
    pull: {
