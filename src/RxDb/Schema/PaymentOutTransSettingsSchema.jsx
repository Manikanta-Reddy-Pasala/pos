...
export const PaymentOutTransSettingsSchema = {
  title: 'Payment Out Transaction Settings Schema table',
  description: '',
  version: 0,
  type: 'object',
  properties: {
...
const pullQueryBuilderInBackground = greenlet(async (doc, localStoragePosId, businessData) => {
  ...
  const query = `{
    paymentOutTransactionSettings(lastId: "${doc.businessId}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
      businessId
      businessCity
      updatedAt
      posId
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setPaymentOutTransactionSettings($input: PaymentOutTransactionSettingsInput) {
...
export const paymentOutTransactionSettingsQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.paymentouttransactionsettings.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validatePaymentOutTransactionSettingsDocumentBeforeSync(
          doc
        );
      }
    },
