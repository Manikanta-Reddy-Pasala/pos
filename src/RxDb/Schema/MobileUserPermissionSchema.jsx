...
export const MobileUserPermissionsSchema = {
  title: 'mobile users table',
  description: 'mobile users list',
  version: 1,
  type: 'object',
  properties: {
    businessId: {
...
const pullQueryBuilderInBackground = greenlet(async (doc, localStoragePosId, businessData) => {
  ...
  const query = `{
        getMobileUserPermissions(lastId: "${doc.userName}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE}, businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
          businessId
          businessCity
          name
          userName
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setMobileUserPermissions($input: MobileUserPermissionsInput) {
...
export const mobileUserPermissionsSyncQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.mobileuserpermissions.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validateMobileUserPermissionsDocumentBeforeSync(
          doc
        );
      }
    },
