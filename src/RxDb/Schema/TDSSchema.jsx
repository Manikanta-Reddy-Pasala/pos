...
export const TDSSchema = {
  title: 'tds table',
  description: 'List of tds txn',
  version: 2,
  type: 'object',
  properties: {
    businessId: {
...
const pullQueryBuilderInBackground = greenlet(async (doc, businessData, localStoragePosId) => {
  ...
  const BATCH_SIZE = 30;
  const query = `{
    getTDS(lastId: "${doc.id}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
      id
      code
      name
      rate
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setTDS($input: TDSInput) {
      setTDS(tds: $input) {
        id,
        updatedAt
      }
...
export const tdsSyncQueryBuilder = (db, syncURL, batchSize, posId, token) => {
  return db.tds.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validateTDSDocumentBeforeSync(doc);
      }
    },
    pull: {
