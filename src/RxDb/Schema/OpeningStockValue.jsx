...
export const OpeningStockValueSchema = {
  title: '',
  description: '',
  version: 0,
  type: 'object',
...
const pullQueryBuilderInBackground = greenlet(async (doc, localStoragePosId, businessData) => {
  ...
  const query = `{
      getOpeningStockValue(lastId: "${doc.id}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE}, businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
        businessId
        businessCity
        stockValue
        stockValueWithTax
...
//   }
//   console.debug('inside push query openingstockvalue::', doc);

//   query = `
//       mutation setStockValue($input: StockValueInput) {
//         setStockValue(stockValue: $input) {
//               updatedAt
//           }
//       }
//   `;
...
export const openingStockValueSyncQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.openingstockvalue.syncGraphQL({
    url: syncURL,
    // headers: {
