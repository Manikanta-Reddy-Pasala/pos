...
export const PurchaseOrderTransactionSettingsSchema = {
  title: 'Purchase Order Transaction Settings Schema table',
  description: '',
  version: 2,
  type: 'object',
  properties: {
...
const pullQueryBuilderInBackground = greenlet(async (doc, localStoragePosId, businessData) => {
  ...
  const query = `{
      purchaseOrderTransactionSettings(lastId: "${doc.businessId}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
        businessId
        businessCity
        updatedAt
        posId
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setPurchaseOrderTransactionSettings($input: PurchaseOrderTransactionSettingsInput) {
...
export const purchaseOrderTransactionSettingsQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.purchaseordertransactionsettings.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validatePurchaseOrderTransactionSettingsDocumentBeforeSync(
          doc
        );
      }
    },
