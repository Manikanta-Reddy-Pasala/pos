...
export const WarehouseSchema = {
  ...
  description: 'List of warehouse txn',
  version: 2,
  type: 'object',
  properties: {
    businessId: {
...
const pullQueryBuilderInBackground = greenlet(async (doc, businessData, localStoragePosId) => {
  ...
  const BATCH_SIZE = 30;
  const query = `{
    getWarehouse(lastId: "${doc.id}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
      id
      name
      updatedAt
      businessId
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setWarehouse($input: WarehouseInput) {
...
export const warehouseSyncQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.warehouse.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validateWarehouseDocumentBeforeSync(doc);
      }
    },
    pull: {
