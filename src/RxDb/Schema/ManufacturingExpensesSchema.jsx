...
export const ManufacturingExpensesSchema = {
  title: 'manufacture expense table',
  description: 'List of manufacture expense txn',
  version: 0,
  type: 'object',
  properties: {
    businessId: {
...
const pullQueryBuilderInBackground = greenlet(async (doc, localStoragePosId, businessData) => {
  ...
  const query = `{
    getManufacturingDirectExpenses(lastId: "${doc.id}", lastUpdatedAt: ${doc.updatedAt}, posId: ${doc.posId}, limit: ${BATCH_SIZE},   businessId: "${doc.businessId}", businessCity: "${doc.businessCity}") {
      id
      name
      updatedAt
      businessId
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setManufacturingDirectExpenses($input: ManufacturingDirectExpensesInput) {
...
export const manufacturingDirectExpensesSyncQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.manufacturedirectexpenses.syncGraphQL({
    url: syncURL,
    // headers: {
    ...
    push: {
       ...
       */
      modifier: async (doc) => {
        return await schemaSync.validateManufacturingDirectExpensesDocumentBeforeSync(
          doc
        );
      }
    },
