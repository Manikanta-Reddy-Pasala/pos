...
export const SyncErrorSchema = {
  title: 'sync error schema table',
  description: 'sync error schema',
  version: 2,
  type: 'object',
  properties: {
    id: {
  ...
  indexes: ['updatedAt'],
  required: ['businessId', 'businessCity', 'tableName', 'updatedAt']
};
...
const pushQueryBuilderInBackground = greenlet(async (doc, localStoragePosId) => {
  ...
  const query = `
    mutation setSyncError($input: SyncErrorInput) {
      setSyncError(syncerror: $input) {
        updatedAt
      }
    }
  `;
...
export const syncErrorSchemaSyncQueryBuilder = (
  db,
  syncURL,
  batchSize,
  posId,
  ...
  return db.syncerror.syncGraphQL({
    url: syncURL,
    // headers: {
